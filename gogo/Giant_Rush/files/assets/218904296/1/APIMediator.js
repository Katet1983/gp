class APIMediator {
    ...
    static async gamePause() {
        if (!APIMediator.isDebugMode()) {
            await window.GameInterface.gamePause();
        }
    }

    static async gameResume() {
        if (!APIMediator.isDebugMode()) {
            await window.GameInterface.gameResume();
        }
    }
    ...
    static setStorageItem(key, value) {
        if (APIMediator.isDebugMode()) return window.localStorage.setItem(key, value);
        return window.GameInterface.storage.setItem(key, value);
    }

    static getStorageItem(key) {
        if (APIMediator.isDebugMode()) return window.localStorage.getItem(key);
    ...
    static initCallbacks() {
        ...
        window.GameInterface.onRestartGame(() => {
            APIMediator.getApp().fire(Events.OnResetLevel);
        });
        ...
        window.GameInterface.onGoToHome(() => {
            ...
            APIMediator.gameQuit().then(() => {
                APIMediator.getApp().fire(Events.OnResetLevel);
                APIMediator.getApp().fire("changeMenuState", MenuManager.States.Home);
            });
        });

        window.GameInterface.onGoToNextLevel(() => {
            /* go to next level */
            LevelGenerator.instance.loadNextLevel();
        });

        window.GameInterface.onGoToLevel(level => {
            LevelGenerator.instance.loadCustomLevel(level);
        });

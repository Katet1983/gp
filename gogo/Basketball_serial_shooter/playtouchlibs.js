...
*********************************************/
	...
	*********************************************/
	...
	arrayWaitForFunction.prototype.version = "1.1.3";
	...
	*********************************************/
	...
	arrayWaitForFunction.prototype.update = function(dt){
		...
		for (var i = 0; i < this.arrayJs.length; i++) {
			this.arrayJs[i].time -= dt;
		};
		this.checkArrayEndOfTime();
	};

	arrayWaitForFunction.prototype.tick = function() {
		this.update(this.C2_RUNTIME.getDt());
	};
	...
	arrayWaitForFunction.prototype.checkArrayEndOfTime = function(){
		...
		if(typeof(toDestroy) == 'undefined'){
			for (var i = 0; i < this.arrayJs.length; i++) {
				if(this.arrayJs[i].time <= 0){
					toDestroyJs = this.arrayJs.splice(i,1)[0];
					break;
				}
			};
		}
		...
		if(typeof(toDestroyJs) != 'undefined'){
			toDestroyJs.callback(toDestroyJs.param);
			this.checkArrayEndOfTime();
		}
	};
...
*********************************************/
	...
	*********************************************/
	...
	timeNotifier.prototype.version = "1.0.5";
...
*********************************************/
	...
	*********************************************/
	...
	leaderboard.prototype.version = "1.0.3";
...
*********************************************/
	...
	*********************************************/
	...
	achievement.prototype.version = "1.1.2";
...
*********************************************/
	...
	playtouch.arrayWaitForFunction = new arrayWaitForFunction().init();
	...
	playtouch.textParser = textParser;
	playtouch.getNew = getNew;
	playtouch.regExp = regExp;

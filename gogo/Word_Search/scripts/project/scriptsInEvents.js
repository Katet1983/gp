...
const scriptsInEvents = {
	...
	async Global_Event43_Act1(runtime, localVars)
	{
	...
	async Function_Event24_Act4(runtime, localVars)
	{
		await crazysdk.displayAd("midgame");
	},

	async Function_Event25_Act5(runtime, localVars)
	{
		await crazysdk.displayAd("rewarded");
	},

	async E_levelcomplate_Event2_Act1(runtime, localVars)
	{
		GameSnacks.score.update(runtime.globalVars.levelno+100);
	},

	async E_levelcomplate_Event2_Act3(runtime, localVars)
	{
		GameSnacks.game.levelComplete(runtime.globalVars.levelno);
	},

	async E_main_Event2_Act2(runtime, localVars)
	{
		GameSnacks.game.ready();
	},

	async E_main_Event2_Act4(runtime, localVars)
	{
		gameLoop(runtime)
	},

	async E_main_Event12_Act1(runtime, localVars)
	{
		crazysdk.gameplayStart();
	},

	async Function1_Event212_Act2(runtime, localVars)
	{
		var jsonString = runtime.globalVars.ajaxData;
		GameSnacks.storage.setItem('wordsearchgameuserdata',jsonString);
	},

	async Function1_Event214_Act1(runtime, localVars)
	{
		let starsEarned =(GameSnacks.storage.getItem('wordsearchgameuserdata'));
		runtime.globalVars.Datastring = starsEarned;
	},

	async E_gameplay_Event101_Act5(runtime, localVars)
	{
		GameSnacks.game.onPause();
	},

	async E_gameplay_Event103_Act2(runtime, localVars)
	{
		GameSnacks.game.onResume();
	},

	async E_gameplay_Event104_Act1(runtime, localVars)
	{
		htmlAds(runtime.globalVars.adtype2,runtime)
	},

	async E_gameplay_Event138_Act1(runtime, localVars)
	{
		crazysdk.gameplayStop();
	},

	async E_crazyads_Event1_Act6(runtime, localVars)
	{
		crazysdk.gameplayStart();
	}

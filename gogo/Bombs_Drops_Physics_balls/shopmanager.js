	...
	*********************************************/
		...
		*/
		shopManager.prototype.init = function(_json){
			...
			_json = (typeof _json === "string") ? JSON.parse(_json) : _json;
		...
		shopManager.prototype.saveStash = function(_stash) {
			_stash = (typeof _stash === "string") ? JSON.parse(_stash) : _stash;
		...
		shopManager.prototype.loadStash = function(_shop,_stash) {
			_shop = (typeof _shop === "string") ? JSON.parse(_shop) : _shop;
			_stash = (typeof _stash === "string") ? JSON.parse(_stash) : _stash;
	
			var _shopInited = JSON.parse(this.init(_shop));
	
			Object.assign(_shopInited.stash.items,_stash.stash.items);
			Object.assign(_shopInited.stash.money,_stash.stash.money);
			for(var i in _shopInited.stash.items){
				if(!_shop.shop[i]){continue;}
				_shopInited.stash.items[i].quantity = Math.min(_shopInited.stash.items[i].quantity,_shop.shop[i].quantityMax);
			}

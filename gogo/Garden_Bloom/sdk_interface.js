	...
	famobi: {
		...
		*/
		...
		localStorage: {
			savegame: {},
			init: function() {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] init()");
				let savegame = GameSnacks.storage.getItem('savegame');
				try {
			...
			update: function() {
				GameSnacks.storage.setItem('savegame', JSON.stringify(this.savegame));
			},
			setItem: function(key, value) {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] setItem('%s', %s)", key, value);
				this.savegame[key] = value;
				this.update();
			},
			getItem: function(key) {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] getItem('%s')", key);
				return this.savegame[key] || null;
			},
			removeItem: function(key) {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] removeItem('%s')", key);
				delete this.savegame[key];
				this.update();
			},
			getKeys: function() {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] getKeys()");
				return Object.keys(this.savegame);
			},
			clear: function() {
				SDK_INTERFACE.settings.debugLevel && console.debug("[localStorage] clear()");
				this.savegame = {};
...
const SDK_INTERFACE_INIT = function() {
	return new Promise(function(resolve, reject) {
		...
		let intervalLocalStorage = setInterval(() => {
			if(typeof window.famobi.localStorage.init === "function") {

...
SDK_INTERFACE_SETTINGS.version = "1.0.4";
		...
		*/
		...
		if(typeof params.eventId === "string") {
			switch(params.eventId.toLowerCase()) {
				...
				case "button:menu:theme":
				case "button:levelselect:play":
				case "button:results:restart":
				case "button:game:restart":
				...
				case "button:quit":
				case "button:menu:start":
				case "button:pause:menu":
				case "button:pause:quit":
				case "button:results:quit":
				case "button:game:achievements":
	...
	return new Promise(function(resolve, reject) {
		...
		switch(event) {
			...
			case "EVENT_LEVELSUCCESS":
				if(params.levelName === "PLAYER_VS_CPU") {
					SDK_INTERFACE_HELPERS.GameSnacks.game.levelComplete(SDK_INTERFACE_HELPERS.getLevel(params.levelName));
				}
				break;
			...
			case "EVENT_PAUSE":
				return window.famobi.showInterstitialAd("button:game:settings", () => {

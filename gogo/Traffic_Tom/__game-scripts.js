    ...
    famobi.getFeatureProperties = function(key) {
        if(key === 'forced_mode') {
            return {
                    ...
                    "override": {
                        "hide_ui": ["levelObjective", /*"progressBar", "speed",*/ "timeRequired", "score"],
                    }
                };
        } else {
            return {};
...
var skipTitleScreen = function() {
    return (typeof famobi !== "undefined" && famobi.hasFeature("skip_title")) || isForcedMode();
};
...
Object.defineProperty(tr.Storage, "mission", {
    ...
    set: function(mission) {
        ...
        var levelNum = this.level - 1;
        
        if (levelNum < 12)
            this.gameArea = tr.Keys.GAME_AREAS.HIGHWAY;
        else if (levelNum < 24)
            this.gameArea = tr.Keys.GAME_AREAS.ROADSIDE;
        else
            this.gameArea = tr.Keys.GAME_AREAS.ROUTE66;
...
ScreenController.prototype.loadScreen = function(screenName, data) {
    var onCurrentScreenHidden = function () {
        ...
        this.loadAssets(screenName.toLowerCase(), function () {
            this.app.fire(tr.Events.SCREEN_LOADED, screenName);
...
ScreenController.prototype.loadAssets = function(tag, loadCallback) {
    ...
    if (tag == 'game') {
        ...
        if (tr.Storage.gameMode == tr.Keys.GAME_MODES.RACE)
            query.push(tr.Storage.mission.boss, 'boss_fight', 'bike_' + BotBike.getBotBikeModelNumber() + '_parts');
    } else if (tag == 'showup') {
...
MapScreen.prototype.initialize = function() {
    ...
    var hud = this.entity.root.findByName('HUD'),
        ...
        onEnable = function () {
            ...
            if (isForcedMode()) {
                //set level
                if (isForcedMode() && getForcedModeProperties() && getForcedModeProperties().state.level)
                    tr.Storage.currentMission = getForcedModeProperties().state.level;
                
                setMission.call({self: this, levelIndex: tr.Storage.currentMission - 1});
                
                var bikeIndex, bikeLevel;
                
                //select minimum required bike
                bikeSelector:
                for (bikeIndex = 0; bikeIndex < tr.Storage.bikes.length; bikeIndex++) {
                    for (bikeLevel = 0; bikeLevel < 4; bikeLevel++) {
                        if (tr.Storage.getMaxSpeedUpgradedByIndex(bikeIndex, bikeLevel) >= tr.Storage.mission.entryBikeSpeed) {
                            break bikeSelector;
                        }
                    }
                }
                
                tr.Storage.selectedBike = bikeIndex + 1;
                tr.Storage.bikeLevels[bikeIndex] = bikeLevel;
...
GameController.attributes.add("speedGauge", {type: "entity"});
GameController.attributes.add("speedHUD", {type: "entity"});
GameController.attributes.add("timeGauge", {type: "entity"});
...
GameController.prototype.initialize = function() {
    ...
    var onEnable = function () {
        ...
        this.missionProgressHUD.enabled = !isUIHidden('progressBar');
        this.goalDetailHUD.enabled = !isUIHidden('levelObjective');
        this.speedGauge.enabled = !isUIHidden('speed');
        this.timeGauge.enabled = !isUIHidden('timeRequired');
...
GameController.prototype.update = function(dt) {
    ...
    if (Math.floor(tr.Storage.distance) != this.lastReportedLiveScore) {
        ...
        Apicontroller.reportLiveScore(this.lastReportedLiveScore);
    }
...
pc.script.createLoadingScreen(function (app) {
    ...
    var onScreenLoad = function(screenName) {
        var targetScreen = isForcedMode() ? tr.Keys.SCREENS.GAME : tr.Keys.SCREENS.GARAGE;
        
        if (screenName === targetScreen) {
            hideSplash();
            
            app.off(tr.Events.SCREEN_LOADED, onScreenLoad);
    ...
    app.on('start', function() {
        ...
        if (skipTitleScreen()) {
            app.on(tr.Events.SCREEN_LOADED, onScreenLoad);
        } else {
            /* hide preloader */
            hideSplash();
...
GameSettings.prototype.initialize = function() {
    ...
    this.homeBtn.enabled = !isForcedMode();

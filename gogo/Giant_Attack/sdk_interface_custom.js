...
SDK_INTERFACE_SETTINGS.version = "1.0.2";
		...
		*/
		...
		if(typeof params.eventId === "string") {
			switch(params.eventId.toLowerCase()) {
				...
				case "button:settings:tutorial":
				case "button:settings:replay":
				case "button:fail:replay":
				case "button:map:level":
				case "button:map:start":
				...
				case "button:map:settings":
				case "button:game:shop":
				...
				case "button:break:start":
				case "button:win:next":
...
SDK_INTERFACE_OVERRIDES.famobi_analytics.trackEvent = (event, params) => {
	...
	return new Promise(function(resolve, reject) {
		...
		switch(event) {
			...
			case "EVENT_CUSTOM":
				if(params.eventName === "LEVELEND" && params.result === "success") {

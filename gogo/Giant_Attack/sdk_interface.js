...
const SDK_INTERFACE_HELPERS = {
	...
	showPauseOverlay: function(isVisible) {
		...
		if(pauseOverlay === null) {
	        ...
	        const canvasContainer = document.getElementById("application-canvas");
	    	if(canvasContainer) {
	...
	GameSnacks: {
		game: {
			...
			levelComplete: function(level) {
				...
				if(["color-pixel-art-classic", "parking-rush", "emoji-fun", "giant-attack"].includes(window.famobi_gameID)) {
					this.levelCompletionScore++;
					SDK_INTERFACE_HELPERS.GameSnacks.score.update(this.levelCompletionScore);
				}
			},
		...
		storage: {
			...
			getItem: function(key) {
				...
				// console.log("%c[GAMESNACKS] GameSnacks.storage.getItem('%s')", 'background: red; color: #bada55', key);
				let result = window.GameSnacks.storage.getItem(key);
				// console.log("%c[GAMESNACKS] Result: %s", 'background: green; color: #bada55', JSON.stringify(result));
				return result;
			},
			...
			setItem: function(key, value) {
				...
				// console.log("%c[GAMESNACKS] GameSnacks.storage.setItem('%s', '%s')", 'background: red; color: #bada55', key, value);
	...
	famobi: {
		...
		*/
		...
		gameReady: function() {
			...
			if(["tile-journey"].includes(window.famobi_gameID)) {
		...
		window.famobi_gameJS.push(() => {
			SDK_INTERFACE_HELPERS.GameSnacks.audio.subscribe(isEnabled => {
				...
				if(SDK_INTERFACE_HELPERS.GameSnacks.audio.isEnabled()) {
					window["TRESOR"].removeItem("muted");
				} else {
					window["TRESOR"].setItem("muted", true);
				}
				*/

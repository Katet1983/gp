function CLevelMenu(){
    ...
    this._init = async function(){
        ...
        if (level_foosball!=="undefined"&&level_foosball!==null){
            level_foosball = parseInt(level_foosball);
            s_iLastLevel = level_foosball;
        }else{
            s_iLastLevel = 1;
        }
        _iCurPage = 0;
	...
	_oContainer.addChild(oBg);
        ...
        _oTextLevel = new CTLText(s_oStage, 
                    CANVAS_WIDTH/2-500, 120, 1000, 250, 
	...
	_pStartPosExit = {x:CANVAS_WIDTH - (oSprite.width/2)-10-30,y:(oSprite.height/2)+10 + 30};
        ...
        if (_fRequestFullScreen && screenfull.enabled){
            ...
            _pStartPosFullscreen = {x: oSprite.width/4 + 10+30,y:(oSprite.height/2)+10 + 30222222};
        }
        ...
        if(_aContainerPage.length > 1){
            ...
            _oArrowRight = new CGfxButton(CANVAS_WIDTH/2+300 ,CANVAS_HEIGHT/2+250-70,s_oSpriteLibrary.getSprite('arrow'),s_oStage);
            _oArrowRight.getButtonImage().rotation = 90;
            _oArrowRight.addEventListener(ON_MOUSE_UP, this._onRight, this);
            
            _oArrowLeft = new CGfxButton(CANVAS_WIDTH/2 - 300,CANVAS_HEIGHT/2 + 250-70,s_oSpriteLibrary.getSprite('arrow'),s_oStage);
    ...
    this._onButLevelRelease = function(iLevel){
        console.log(iLevel);
        LevelMode = true;
        if (Number(s_iLastLevel) >= iLevel+1) {

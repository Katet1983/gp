...
var bootState = {
	...
	init: function(){
		...
		window.famobi.onRequest("pauseGameplay", () => {
			if(typeof playState !== "undefined") {
				playState.gameInfo.gameRunning = false;
			}
		});

		window.famobi.onRequest("resumeGameplay", () => {
			if(typeof playState !== "undefined") {
				playState.gameInfo.gameRunning = true;
			}
		});

		//window.famobi_analytics.trackScreen("SCREEN_SPLASH"); // Moved to 'create' function below, as it was causing an error here.
		...
		document.getElementsByTagName("body")[0].style.backgroundColor = '#687C63';
	...
	preload: function(){
		...
		this.load.image('loadPanel', 'assets/img/loadPanel.png');

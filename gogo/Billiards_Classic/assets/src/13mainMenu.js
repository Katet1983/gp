var menuState = {
	...
	init: function(){
		...
		if(window.famobi.localStorage.getItem('tutorialPlayed') == null){
			//console.log("no data found - setting new data")
			projectInfo.tutorialPlayed = 0;
			try{
				window.famobi.localStorage.setItem('tutorialPlayed', 1)
			}catch(e){

			}

		}else{
			projectInfo.tutorialPlayed = window.famobi.localStorage.getItem('tutorialPlayed');
			//console.log("data found: " + projectInfo.bestScore)
	...
	create: function(){
		...
		menuInfo.menuCanvas.addChild(menuInfo.pVpButton);
		...
		menuInfo.settingsButton = new Phaser.Button(game, 0, 0, 'settingsButton', () => {
			window.famobi.showInterstitialAd("button:main:settings", () => {
				onSettingsClick();
			});	
		}, this, 1, 0, 1, 0);
		...
		*/
		...
		//new for 1.1 - instead of fading menu up, jump straight to game
		...
		function initGame(){
			...
			if(projectInfo.tutorialPlayed == 0){

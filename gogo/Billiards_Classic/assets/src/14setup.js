...
playState.init = function(){
	...
	//this version only - no two player mode.
	projectInfo.mode = 1;
...
playState.create = function(){
	...
	resizeGame = function(scale, bounds){
		...
		if(window.famobi.getOrientation() == "portrait"){
			...
			gameInfo.menuButton.x = game.width - 110;
			...
			gameInfo.GOhighScoreIcon.x = 40;
			gameInfo.GOhighScoreIcon.y = -366;
			...
			if(!window.famobi.hasFeature("external_mute")) {
				gameInfo.replayButtonPU.y = -400;
				gameInfo.muteButtonPU.y = -200;
				gameInfo.aiButtonPU.y = 0;
				gameInfo.guideButton.y = 200;
				gameInfo.helpButton.y = 400;
			} else {
				gameInfo.replayButtonPU.y = -300;
				gameInfo.aiButtonPU.y = -100;
				gameInfo.guideButton.y = 100;
				gameInfo.helpButton.y = 300;
			}

			gameInfo.replayButtonPU.x = 0;
			gameInfo.muteButtonPU.x = 0;
			gameInfo.aiButtonPU.x = 0;
			gameInfo.guideButton.x = 0;
			gameInfo.helpButton.x = 0;


			//icons
			gameInfo.humanIcon.x = 170;
			gameInfo.humanIcon.y = game.height - 110;
			gameInfo.humanIcon.scale = new Point(.9, .9);


			gameInfo.aiIcon.x = 170;
			gameInfo.aiIcon.y = game.height - 15;
			gameInfo.aiIcon.anchor = new Point(0, 1);
			gameInfo.aiIcon.scale = new Point(.9, .9);

			gameInfo.turnArrow1.x = 125;
			gameInfo.turnArrow1.y = game.height - 125;

			gameInfo.turnArrow2.x = 125;
			gameInfo.turnArrow2.scale = new Point(1, 1);
			gameInfo.turnArrow2.y = game.height - 35;
			...
			*/
			...
			gameInfo.timerText.x = gameInfo.guiPanel3.x + gameInfo.guiPanel3.width / 2;
			gameInfo.timerText.y = game.height - 45;

			gameInfo.scoreText.x = game.width / 2,
			gameInfo.scoreText.y = game.height - 43;

			gameInfo.multiplierText.x = gameInfo.guiPanel1.x - gameInfo.guiPanel1.width / 2;
			gameInfo.multiplierText.y = game.height - 45;
			...
			//icons
			gameInfo.humanIcon.x = 160;
			gameInfo.humanIcon.y = game.height - 35;
			gameInfo.humanIcon.anchor = new Point(0, 1);
			gameInfo.humanIcon.scale = new Point(1, 1);

			gameInfo.aiIcon.x = game.width - 160 - gameInfo.aiIcon.width;
			gameInfo.aiIcon.y = game.height - 35;
			gameInfo.aiIcon.anchor = new Point(0, 1);
			gameInfo.aiIcon.scale = new Point(1, 1);

			gameInfo.turnArrow1.x = 105;
			gameInfo.turnArrow1.y = game.height - 55;

			gameInfo.turnArrow2.x = game.width - 105;
			gameInfo.turnArrow2.y = game.height - 55;
			gameInfo.turnArrow2.scale = new Point(-1, 1);
			...
			*/
			...
			gameInfo.GOhighScoreIcon.x = 180;
			gameInfo.GOhighScoreIcon.y = -265;
			gameInfo.gameOverPanel.text2.x = 280;
			...
			if(!window.famobi.hasFeature("external_mute")) {
				gameInfo.replayButtonPU.x = -400;
				gameInfo.muteButtonPU.x = -200;
				gameInfo.aiButtonPU.x = 0;
				gameInfo.guideButton.x = 200;
				gameInfo.helpButton.x = 400;
			} else {
				gameInfo.replayButtonPU.x = -300;
				gameInfo.aiButtonPU.x = -100;
				gameInfo.guideButton.x = 100;
				gameInfo.helpButton.x = 300;
			}

			gameInfo.replayButtonPU.y = 0;
			gameInfo.muteButtonPU.y = 0;
			gameInfo.aiButtonPU.y = 0;
			gameInfo.guideButton.y = 0;
			gameInfo.helpButton.y = 0;
	...
	function setTurn(){
		...
		if(projectInfo.tutorial == false){
			...
			if(projectInfo.lastBreaker == "none"){
				...
				if(Math.random() < 0.5){
					gameInfo.turn = "p1";
					gameInfo.turnArrow1.visible = true;
					...
					gameInfo.turnArrow2.visible = true;
				}
			}else{
	...
	function initGUI(){
		...
		for(var n = 0; n < 7; n ++){
			...
			gameInfo.rackSolidsArray[n] = new Ball(35, n + 1);
			gameInfo.rackSolidsArray[n].ballRotation = new Array(1, 0, 0, 0);
			gameInfo.rackSolidsArray[n].renderBall(gameInfo.rackSolidsArray[n].ballRotation);


			gameInfo.rackSolids.addChild(gameInfo.rackSolidsArray[n]);
			gameInfo.rackSolidsArray[n].x = -152 + n * 50;
			gameInfo.rackSolidsArray[n].y = -47;
		...
		gameInfo.rackSolids8ball  = new Ball(35, 8);

		gameInfo.rackSolids.addChild(gameInfo.rackSolids8ball);
		gameInfo.rackSolids8ball.x = -152;
		gameInfo.rackSolids8ball.y = -47;
		...
		for(var n = 0; n < 7; n ++){
			...
			gameInfo.rackStripesArray[n] = new Ball(35, n + 9);
			gameInfo.rackStripesArray[n].ballRotation = new Array(1, 0, 0, 0);
			gameInfo.rackStripesArray[n].renderBall(gameInfo.rackStripesArray[n].ballRotation);

			gameInfo.rackStripes.addChild(gameInfo.rackStripesArray[n]);
			gameInfo.rackStripesArray[n].x = -152 + n * 50;
			gameInfo.rackStripesArray[n].y = -47;
		...
		gameInfo.rackStripes8ball  = new Ball(35, 8);

		gameInfo.rackStripes.addChild(gameInfo.rackStripes8ball);
		gameInfo.rackStripes8ball.x = -152;
		gameInfo.rackStripes8ball.y = -47;
		...
		gameInfo.multiplierText = new Phaser.BitmapText(game, 0, 0, 'font7', "x" + gameInfo.multiplier, 56);
		...
		gameInfo.menuButton = new Phaser.Button(game, 0, 0, 'menuButton', () => {
		...
		gameInfo.aiWin = new Phaser.Sprite(game, -120, -80, 'aiWin');
		...
		gameInfo.GOscoreIcon = new Phaser.Sprite(game, -70, -195, 'score');
		gameInfo.gameOverPanel.addChild(gameInfo.GOscoreIcon);

		gameInfo.GOhighScoreIcon = new Phaser.Sprite(game, 220, -280, 'highScore');
		gameInfo.gameOverPanel.addChild(gameInfo.GOhighScoreIcon);

		gameInfo.GOaiLevel = new Phaser.Sprite(game, 75, 0, 'aiIcon');
		gameInfo.GOaiLevel.anchor = new Point(.5, .5);
		gameInfo.gameOverPanel.addChild(gameInfo.GOaiLevel);
		gameInfo.GOratingText = new Phaser.BitmapText(game, 0, 0, 'font6', projectInfo.aiRating.toString(), 38);
		gameInfo.GOaiLevel.addChild(gameInfo.GOratingText);
		gameInfo.GOratingText.anchor = new Point(.5, .5);
		//gameInfo.GOaiLevel.scale = new Point(.5, .5);

		gameInfo.GOclockIcon = new Phaser.Sprite(game, -194, -53, 'clockIcon');
		...
		//restart game
		gameInfo.replayButton = new Phaser.Button(game, 0, 100, 'playButtonSmall', () => {
			window.famobi.showInterstitialAd("button:result:restart", () => {
				replayGame();
			});
		}, this, 1, 0, 1);
		gameInfo.replayButton.anchor = new Point(0.5, 0);
		...
		//restart game
		gameInfo.replayButtonPU = new Phaser.Button(game, -0, 0, 'replayButton', () => {
		...
		//mute button
		gameInfo.muteButtonPU = new Phaser.Button(game, 0, 0, 'muteButton', toggleSound, this, 1, 0, 1, 0);
		...
		gameInfo.aiButtonPU = new Phaser.Button(game, -0, 0, 'aiButton', () => {
			increaseAI();
		}, this, 1, 0, 1, 0);
		gameInfo.popUpPanel.addChild(gameInfo.aiButtonPU);
		gameInfo.aiButtonPU.anchor = new Phaser.Point(.5, .5);
		
		gameInfo.ratingText = new Phaser.BitmapText(game, 0, -8, 'font6', projectInfo.aiRating.toString(), 40);
		gameInfo.aiButtonPU.addChild(gameInfo.ratingText);
		gameInfo.ratingText.anchor = new Point(.5, .5);

		gameInfo.guideButton = new Phaser.Button(game, -0, 0, 'guideButton', toggleGuide, this, 1, 0, 1, 0);
		gameInfo.popUpPanel.addChild(gameInfo.guideButton);
		gameInfo.guideButton.anchor = new Phaser.Point(.5, .5);

		gameInfo.helpButton = new Phaser.Button(game, -0, 0, 'helpButton', showHelp, this, 1, 0, 1, 0);
		gameInfo.popUpPanel.addChild(gameInfo.helpButton);
		gameInfo.helpButton.anchor = new Phaser.Point(.5, .5);
		...
		gameInfo.aiButtonPU.input.enabled = false;
		gameInfo.guideButton.input.enabled = false;
		gameInfo.helpButton.input.enabled = false;
		...
		if(projectInfo.tutorial == true){
			...
			gameInfo.guiPanel1.visible = false;
			gameInfo.guiPanel2.visible = false;
			gameInfo.guiPanel3.visible = false;
	...
	function increaseAI(){
		...
		gameInfo.ratingText.text = projectInfo.aiRating.toString();
		try{
	...
	function toggleGuide(){
		...
		if(projectInfo.guideOn == 0){
			...
			gameInfo.guideButton.setFrames(1, 0, 1, 0);
			try{
			...
			gameInfo.guideButton.setFrames(3, 2, 3, 2);
	...
	function gameToPopUpMenu(){
		...
		if(gameInfo.foulWindow.visible != true && gameInfo.gameOver == false){
			...
			gameInfo.aiButtonPU.input.enabled = true;
			gameInfo.guideButton.input.enabled = true;
			gameInfo.helpButton.input.enabled = true;
	...
	function replayGame(){
		...
		window.famobi_analytics.trackEvent("EVENT_LEVELFAIL", {levelName: projectInfo.levelName, reason: "draw"}).then(
				function() {
					game.state.start('play');
				},
				function() {
...
playState.resumeGame = function(){
	...
	gameInfo.popUpPanel.visible = false;
	gameInfo.menuButton.visible = true;

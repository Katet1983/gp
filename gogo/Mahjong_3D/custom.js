	...
	return new Promise(function(resolve, reject) {
		switch(event) {
			...
			case "EVENT_LEVELSTART":
				...
				break;
			case "EVENT_CUSTOM":
				if(params.event = "EVENT_LEVELSUCCESS") {
...
window.famobi.showRewarded = function() {

	return new Promise(function(resolve, reject) {

		if(rewardedAdObject.state !== rewardedAdObject.states.READY) {
			reject("not ready: " + rewardedAdObject.state);
			return;
		}

		if(typeof rewardedAdObject.showAdFn !== "function") {
			reject("showAdFn is not a function");
			return;
		}

		showRewarded_adViewed = function() {
			rewardedAdObject.result.rewardGranted = true;
			resolve(rewardedAdObject.result);
		};

		showRewarded_adDismissed = function() {
			rewardedAdObject.result.rewardGranted = false;
			resolve(rewardedAdObject.result);
		};

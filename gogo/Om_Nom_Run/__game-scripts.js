...
EventTypes.FIRST_PERSON_GOGGLES_START_FLASHING = 'firstPersonGoggles:startFlashing';
EventTypes.FIRST_PERSON_GOGGLES_STOP_FLASHING = 'firstPersonGoggles:stopFlashing';
 ...
 **/
pc.AppBase.prototype.raycastAllAdvanced = function(from, to, results) {
    results = results || [];
...
pc.AppBase.prototype.raycastAll = function(from, to, results) {
    ...
    if(!this.raycastFiltered) {
            ...
            pc.AppBase.prototype.raycastFiltered = function(o, i) {
...
Constants.AUDIO = {
    ...
    INVINCIBLE_COLLISION: 'invincibleCollision',
};
...
Constants.Storage = {
    ...
    FREE_RUN_LEADERBOARD: 'freeRunLeaderboarde_v2',
    FREE_RUN_LEADERBOARD_CREATION_TIME: 'freeRunLeaderboardCreationTimee_v2',
    STUNTS_LEADERBOARD: 'stuntsLeaderboarde_v2',
    STUNTS_LEADERBOARD_CREATION_TIME: 'stuntsLeaderboardCreationTime_v2',
    LANGUAGE: 'language',
...
Constants.LEADERBOARD_CHARACTER_NAMES = [
    'leaderboard_Boo',
    'leaderboard_Cheddar',
    'leaderboard_Doggy Granny',
    'leaderboard_Ferret',
    'leaderboard_Gekko',
    'leaderboard_Hyena',
    'leaderboard_Lick',
    'leaderboard_Om Nelle',
    'leaderboard_RoboKid',
    'leaderboard_Sheep',
    'leaderboard_Snowman',
    'leaderboard_Space Cowgirl',
    'leaderboard_Spider',
    'leaderboard_SwordKid',
    'leaderboard_TechKid',
    'leaderboard_You',
];

Constants.LEADERBOARD_AVATARS = {
    'leaderboard_Boo': 'Leaderboard_Avatar_Boo.png',
    'leaderboard_Cheddar': 'Leaderboard_Avatar_Cheddar.png',
    'leaderboard_Doggy Granny': 'Leaderboard_Avatar_DoggyGranny.png',
    'leaderboard_Ferret': 'Leaderboard_Avatar_Ferret.png',
    'leaderboard_Gekko': 'Leaderboard_Avatar_Gekko.png',
    'leaderboard_Hyena': 'Leaderboard_Avatar_Hyena.png',
    'leaderboard_Lick': 'Leaderboard_Avatar_Lick.png',
    'leaderboard_Om Nelle': 'Leaderboard_Avatar_OmNelle.png',
    'leaderboard_RoboKid': 'Leaderboard_Avatar_Robokid.png',
    'leaderboard_Sheep': 'Leaderboard_Avatar_Sheep.png',
    'leaderboard_Snowman': 'Leaderboard_Avatar_Snowman.png',
    'leaderboard_Space Cowgirl': 'Leaderboard_Avatar_SpaceCowgirl.png',
    'leaderboard_Spider': 'Leaderboard_Avatar_Spider.png',
    'leaderboard_SwordKid': 'Leaderboard_Avatar_SwordKid.png',
    'leaderboard_TechKid': 'Leaderboard_Avatar_TechKid.png',
    'leaderboard_You': 'Leaderboard_Avatar_OmNom.png'
}
...
Constants.getLevelEntity = function () {
    if (!Constants._levelEntity) {
        Constants._levelEntity = pc.AppBase.getApplication().root.findByName('Level');
    }
...
GameStateController.prototype.finishLevel = function () {
    ...
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        ...
        progression01: MissionsManager.getInstance().isFreeRunMode() ? 'Endless' : `Level_${MissionsManager.getInstance().getCurrentMissionIndex()}`
    });
...
GameStateController.prototype.onCharacterDead = function () {
    ...
    if (Apicontroller.hasRewardedVideo() && MissionsManager.getInstance().isReviveAvailable()) {
        MissionsManager.getInstance().useRevive();
        ...
        this.entity.delayedCall(isForcedMode() ? 200 : 1000, () => {
            ...
            window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
                eventName: "GA:Progression",
                progressionStatus: showResultsLocation ? "Complete" : "Fail",
                progression01: MissionsManager.getInstance().isFreeRunMode() ? 'Endless' : `Level_${MissionsManager.getInstance().getCurrentMissionIndex()}`
            });
...
GameStateController.prototype.assignEntityMethods = function () {
    ...
    this.level.gameState.isInvincible = function () {
        return this.script.stuntController.stuntActive || this.script.stuntController.waitingForLanding || PowerupManager.getInstance().isActive(Powerups.FIRST_PERSON_VIEW) || this.script.stuntController.landingElapsedTime < 1.5;
    }.bind(this.entity);
...
GameStateController.prototype.exitGameplay = function () {
    if (!MissionsManager.getInstance().isFreeRunMode()) {
        LevelStatsManager.getInstance().saveLevelDistance();
    } 
    this.setGameState(GameState.INACTIVE);
        ...
        this.app.touch.on(pc.EVENT_TOUCHEND, () => {
            if(this.app.soundManager.context.state === 'interrupted' || this.app.soundManager.context.state === 'suspended') {
                this.app.soundManager.context.resume().then(() => {
                    ...
                    console.log('can not resume audio context!')
                });
            }
        });
    }
...
MainMenuUi.prototype.initialize = function () {
    ...
    this.buttonShare = this.entity.findByName('ButtonShare');
    ...
    BasicButton.assignAction(this.buttonShare, this.shareClicked, this);
    ...
    this.buttonShare.enabled = famobi.hasFeature("share");
...
MainMenuUi.prototype.assignEntityMethods = function () {
    ...
    this.entity.show = function (options) {
        ...
        if(options && options.continueMusic) {
            //just continue music instead of restarting
        } else {
...
MainMenuUi.prototype.showLobbyOneRunCharacters = function () {
    if (Apicontroller.isRewardedFeatureEnabled()) {
        this.manageAdditinalCharactersWithRewarded();
    } else {
        this.manageAdditinalCharactersWithoutRewarded();
    }


};

MainMenuUi.prototype.manageAdditinalCharactersWithRewarded = function () {
...
MainMenuUi.prototype.manageAdditinalCharactersWithoutRewarded = function () {
    const characterLeftKey = CharactersManager.getInstance().getOneRunCharacterLeft();
    const characterRightKey = CharactersManager.getInstance().getOneRunCharacterRight();

    if (CharactersManager.getInstance().isLeftCharacterTried()) {
        this.leftCharacterContainer.children.forEach(character => character.enabled = false);
        this.buttonOneRunLeft.enabled = false;
    } else {
        this.leftCharacterContainer.children.forEach(character => character.enabled = character.name === `${characterLeftKey}Preview`);
    ...
    if (CharactersManager.getInstance().isRightCharacterTried()) {
        this.rightCharacterContainer.children.forEach(character => character.enabled = false);
        this.buttonOneRunRight.enabled = false;
    } else {
        this.rightCharacterContainer.children.forEach(character => character.enabled = character.name === `${characterRightKey}Preview`);
...
MainMenuUi.prototype.misisonsClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:Levels",
    });
...
MainMenuUi.prototype.shareClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:Share",
    });
    if(window.SHARE_GAME_CALLBACK) {
        window.SHARE_GAME_CALLBACK();
    } else {
        console.warn('Please define window.SHARE_GAME_CALLBACK function!');
    }
};
...
MainMenuUi.prototype.freeRunClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:FreeRun",
    });
...
MainMenuUi.prototype.luckWheelClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:Loterry",
    });
...
MainMenuUi.prototype.dailyRewardClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:DailyReward",
    });
...
MainMenuUi.prototype.championshipClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:StuntChampionship",
    });
...
MainMenuUi.prototype.oneRunLeftClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:SelectOneRunCharacterLeft",
    });
...
MainMenuUi.prototype.oneRunRightClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:SelectOneRunCharacterRight",
    });
    this.app.fire(EventTypes.SHOW_UI_WINDOW, Constants.UI.CHARACTER_ONE_RUN_LOCATION);
...
MainMenuUi.prototype.upgradeClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MainMenu:CharacterUpgrade",
    });
...
UiController.prototype.showUIWindow = function(windowName, hideOthers = true, options = {}) {
    if(hideOthers) {
        this.hideAllWindows();
    }
...
UiController.prototype.hideAllWindows = function() {   
    this.entity.children.forEach(windowEntity => {
            if(windowEntity.hide && typeof windowEntity.hide === 'function') {
                ...
                if(windowEntity.name !== 'FamobiCopyright') {
...
LocalStorageController.prepareDefaultSaveData = function () {
    ...
    defaultSaveData[Constants.Storage.LEVEL_DISTANCES] = [];
...
LocalStorageController.getActualSaveData = function () {
    ...
    currentSaveData[Constants.Storage.MISSION_STATES] = LocalStorageController._compressMissionsData(MissionsManager.getInstance().getMissionStates());
    currentSaveData[Constants.Storage.ACTIVE_CHARACTER] = CharactersManager.getInstance().getCurrentUnlockedCharacter();
    currentSaveData[Constants.Storage.CHARACTERS_DATA] = LocalStorageController._compressCharactersData(CharactersManager.getInstance().getCharactersData());
    ...
    currentSaveData[Constants.Storage.FREE_RUN_LEADERBOARD] = LocalStorageController._compressLeaderboardData(FreeRunLocation.freeRunLeaderboard);
    currentSaveData[Constants.Storage.FREE_RUN_LEADERBOARD_CREATION_TIME] = FreeRunLocation.freeRunLeaderboardCreationTime;
    currentSaveData[Constants.Storage.STUNTS_LEADERBOARD] = LocalStorageController._compressLeaderboardData(StuntChampionshipLocation.stuntsLeaderboard);
    ...
    currentSaveData[Constants.Storage.LEVEL_DISTANCES] = LevelStatsManager._instance ? LevelStatsManager.getInstance().getLevelDistancesObject() : []

    return currentSaveData;
};

LocalStorageController._compressCharactersData = function (rawData) {
    const keys = Object.keys(rawData);
    const targetObject = {};
    for (let key of keys) {
        const sourceObject = rawData[key];
        targetObject[key] = [sourceObject.levels, +sourceObject.purchased];
    }
    return targetObject;
};

LocalStorageController._decompressCharactersData = function (compressedData) {
    if (!compressedData) return compressedData;
    const keys = Object.keys(compressedData);
    const targetObject = {};
    for (let key of keys) {
        const sourceObject = compressedData[key];
        if (Array.isArray(sourceObject)) {
            targetObject[key] = { levels: sourceObject[0], purchased: !!sourceObject[1] };
        } else {
            targetObject[key] = sourceObject;
...
LocalStorageController._compressMissionsData = function (rawData) {
    return rawData.map(row => [row.missionIndex, row.missionLevel, +row.rewardClaimed]);
};

LocalStorageController._decompressMissionsData = function (compressedData) {
    if (!compressedData) return compressedData;
    return compressedData.map(row => {
        if (Array.isArray(row)) {
            return { missionIndex: row[0], missionLevel: row[1], rewardClaimed: !!row[2] };
        } else {
            return row;
        }
    });
...
LocalStorageController._compressLeaderboardData = function (rawData) {
    return rawData.map(row => [row.name, row.name, +row.score, +row.reward]);
};

LocalStorageController._decompressLeaderboardData = function (compressedData) {
    if (!compressedData) return compressedData;
    return compressedData.map(row => {
        if (Array.isArray(row)) {
            return { name: row[0], avatar: Constants.LEADERBOARD_AVATARS[row[0]], score: +row[2], reward: +row[3] };
        } else {
            return row;
        }
    });
...
LocalStorageController.getLeaderboardSlotKey = function () {
    return Constants.GAME_NAME + Constants.GAME_VERSION + "_leaderboard";
};
...
LocalStorageController.saveData = function (saveDataObject = null, immediately = true) {
    ...
    const performSave = () => {
        ...
        const commonSaveData = Object.keys(LocalStorageController.lastSaveData).filter(objKey =>
            objKey !== Constants.Storage.FREE_RUN_LEADERBOARD && objKey !== Constants.Storage.STUNTS_LEADERBOARD).reduce((newObj, key) => {
                newObj[key] = LocalStorageController.lastSaveData [key];
                return newObj;
            }, {});
        const leaderboardSaveData = Object.keys(LocalStorageController.lastSaveData).filter(objKey =>
            objKey === Constants.Storage.FREE_RUN_LEADERBOARD || objKey === Constants.Storage.STUNTS_LEADERBOARD).reduce((newObj, key) => {
                newObj[key] = LocalStorageController.lastSaveData [key];
                return newObj;
            }, {});

        LocalStorageController.currentLocalStorage.setItem(LocalStorageController.getSlotKey(), JSON.stringify(commonSaveData));
        LocalStorageController.currentLocalStorage.setItem(LocalStorageController.getLeaderboardSlotKey(), JSON.stringify(leaderboardSaveData));
    };
...
LocalStorageController.loadData = function () {
    ...
    var leaderboardData = LocalStorageController.currentLocalStorage.getItem(LocalStorageController.getLeaderboardSlotKey());
...
LocalStorageController.getSavedValue = function (key) {
    if (LocalStorageController.lastSaveData) {
        switch (key) {
            case Constants.Storage.MISSION_STATES:
                return LocalStorageController._decompressMissionsData(LocalStorageController.lastSaveData[key]);
            case Constants.Storage.CHARACTERS_DATA:
                return LocalStorageController._decompressCharactersData(LocalStorageController.lastSaveData[key]);
            case Constants.Storage.FREE_RUN_LEADERBOARD:
                return LocalStorageController._decompressLeaderboardData(LocalStorageController.lastSaveData[key]);
            case Constants.Storage.STUNTS_LEADERBOARD:
                return LocalStorageController._decompressLeaderboardData(LocalStorageController.lastSaveData[key]);
        }
...
MissionReward.prototype.closeClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:MissionReward:Close",
    });
...
MissionPanelBig.prototype.assingEntityMethods = function() {
    ...
    this.entity.buildComponents = function(missionDefinition) {
        ...
        this.backgroundColored.element.spriteAsset = backgroundColored;
        if(LocalizationManager.getInstance().getCurrentLocale() === 'en-US') {
            this.locationIcon.enabled = true;
            this.locationIcon.element.spriteAsset = themeIcon;
        } else {
            this.locationIcon.enabled = false;
        }
...
SettingsPopup.prototype.musicMuteClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: MusicController.musicEnabled ?  "Button:Settings:MuteMusic" :  "Button:Settings:UnmuteMusic"
    });
...
SettingsPopup.prototype.sfxMuteClicked = function() {
     window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: MusicController.sfxEnabled ?  "Button:Settings:MuteSFX" :  "Button:Settings:UnmuteSFX"
    });
...
SettingsPopup.prototype.buttonChooseLanguageClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:Settings:Language"
    });
    this.app.fire(EventTypes.SHOW_UI_WINDOW, Constants.UI.LANGUAGE_SELECTION_POPUP, false);
};
...
RestartLocation.prototype.assignEntityMethods = function () {
    ...
    this.entity.show = function () {
        ...
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            ...
            itemId: "levelReward",
            amount: CoinsStorage.getInstance().getSessionCoins() || 0,
            resourceCurrency: "coin"
        });
            ...
            try {
                Promise.all([
                    window.famobi_analytics.trackEvent(
                        "EVENT_LEVELFAIL",
                        {
                            levelName: `${MissionsManager.getInstance().isFreeRunMode() ? "free-run" : MissionsManager.getInstance().getCurrentMissionIndex()}`,
                            reason: 'dead'
                        }
                    ),
                    window.famobi_analytics.trackEvent(
                        ...
                        {
                            levelName: `${MissionsManager.getInstance().isFreeRunMode() ? "free-run" : MissionsManager.getInstance().getCurrentMissionIndex()}`,
                            levelScore: Math.floor(LevelStatsManager.getInstance().ranDistance)
                        }
                    )
...
RestartLocation.prototype.getExtraCoinsClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:Defeat:GetExtraCoins",
    });

    const addExtraCoins = () => {
        const coinsAmount = Math.max(CoinsStorage.getInstance().getSessionCoins() * 4, 10);

        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            eventName: "GA:Resource",
            flowType: "Source",
            itemType: "levelExtraReward",
            itemId: "levelExtraReward",
            amount: coinsAmount,
            resourceCurrency: "coin"
        });

        CoinsStorage.getInstance().addCoins(coinsAmount);
...
GiveUppopup.prototype.noClicked = function () {
    ...
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:PauseMenu:Cancel",
    });
...
CollectRewardPopup.prototype.claimClicked = function() {
     window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:ClaimReward:Claim",
    });
...
CharacterUpgradeLocation.prototype.previousClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:CharacterUpgrade:Previous"
    });
...
CharacterUpgradeLocation.prototype.nextClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:CharacterUpgrade:Next"
    });
    const currentCharacterIndex = pc.math.clamp(CharactersManager.getInstance().getCharactersConfig().characters.indexOf(this.currentCharacter), 0, CharactersManager.getInstance().getCharactersConfig().characters.length - 1);
...
CharacterUpgradeLocation.prototype.upgradeClicked = function(index) {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:CharacterUpgrade:Upgrade" + (index + 1)
    });
    ...
    if(CoinsStorage.getInstance().hasEnoughCoins(stuntUpgradePrice)) {
        ...
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            ...
            flowType: "Sink",
            itemType: "upgrade",
            itemId: "UpgradeStunts",
            amount: stuntUpgradePrice,
            resourceCurrency: "coin"
        });
    } else {
...
CharacterUpgradeLocation.prototype.unlockClicked = function() {    
    ...
    if (CoinsStorage.getInstance().hasEnoughCoins(this.currentCharacter.unlock))  {
        ...
        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            ...
            flowType: "Sink",
            itemType: "purchase",
            itemId: "UnlockCharacter_" + this.currentCharacter.key,
            amount: this.currentCharacter.unlock,
            resourceCurrency: "coin"
        });

        window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
            eventName: "GA:Design",
            eventId: "Button:CharacterUpgrade:UnlockCharacter:" + this.currentCharacter.key
        });
    }
    this.updateData();
};
...
CharacterUpgradeLocation.prototype.closeClicked = function() {
    ...
    this.app.fire(EventTypes.SHOW_UI_WINDOW, Constants.UI.MAIN_MENU, true, {continueMusic: true});
};
...
StuntChampionshipLocation.prototype.updateStuntsLeaderboard = function(skipProgressCheck) {
    ...
    if(stuntsLeaderboard) {
        ...
        const currentPlayerPosition = stuntsLeaderboard.findIndex(character => character.name === 'leaderboard_You');
        
        /* set player scores */
        stuntsLeaderboard.forEach(character => {
            if (character.name === 'leaderboard_You') {
        ...
        const nextPlayerPosition = stuntsLeaderboard.findIndex(character => character.name === 'leaderboard_You'); 
...
StuntChampionshipLocation.prototype.checkMainCharacter = function() {
    StuntChampionshipLocation.stuntsLeaderboard.forEach(record => {
        if (record.name === 'leaderboard_You') {
...
StuntChampionshipLocation.prototype.createLeaderboard = function() {
    ...
    for (let i = 0; i < StuntChampionshipLocation.stuntsLeaderboard.length; i++) {
        ...
        padsList.forEach(StuntChampionshipLocation.stuntsLeaderboard[i].name === 'leaderboard_You' ? setPadColorPlayer : ((i % 2 === 0) ? setPadColorLight : setPadColorDark));
        ...
        characterName.element.text = LocalizationManager.getInstance().getLocalizedText(StuntChampionshipLocation.stuntsLeaderboard[i].name); // characterName.element.text = StuntChampionshipLocation.stuntsLeaderboard[i].name;
...
StuntChampionshipLocation.prototype.scrollToPlayerAvatar = function() {
    const playerIndex = StuntChampionshipLocation.stuntsLeaderboard.findIndex(element => element.name === 'leaderboard_You');
...
CharacterOneRunLocation.prototype.initialize = function () {
    ...
    this.buttonOneAdIcon = this.buttonOneRun.findByName('IconAd');
    this.buttonOneGreyed = this.entity.findByName('ButtonOneRunGreyed');
    this.buttonOneGreyedAdIcon = this.buttonOneGreyed.findByName('IconAd');
    this.unlockPrice = this.entity.findByName('ButtonUnlock').findByName('Text');
    this.rewardValue = 250;
    ...
    setInterval(() => this.updateButtonsAvailability(), 500);
...
CharacterOneRunLocation.prototype.updateButtonsAvailability = function () {
    ...
    if(Apicontroller.isRewardedFeatureEnabled()) {
        ...
        this.buttonOneRun.enabled = !this.characterObtained;
        this.buttonOneGreyed.enabled = !this.buttonOneRun.enabled;
    }
...
CharacterOneRunLocation.prototype.assignEntityMethods = function () {
    ...
    this.entity.show = function () {
        ...
        this.enabled = true;
        scriptContext.updateButtonsAvailability();

        scriptContext.buttonOneAdIcon.enabled = Apicontroller.isRewardedFeatureEnabled();
        scriptContext.buttonOneGreyedAdIcon.enabled = Apicontroller.isRewardedFeatureEnabled();
...
CharacterOneRunLocation.prototype.selectCharacter = function (characterKey) {
    ...
    this.updateData(name, this.rewardValue, 50000, this.currentCharacterKey);
};
...
CharacterOneRunLocation.prototype.onOneRunClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:OneRun:Rub"
    });
    const addExtraCoinsAndRun = () => {
        if (!Apicontroller.isRewardedFeatureEnabled()) {
            if (CharactersManager.getInstance().getOneRunCharacterLeft() === this.currentCharacterKey) {
                CharactersManager.getInstance().setLeftCharacterTried(true);
            } else if (CharactersManager.getInstance().getOneRunCharacterRight() === this.currentCharacterKey) {
                CharactersManager.getInstance().setRightCharacterTried(true);
            }
        }
...
ChracterRaycastController.prototype.detectFrontCollision = function(dt) {
            ...
            case 'topCollider':
                ...
                if(!this._rampInFront) {                    
                    if(this.entity.laneChangedRecently) {
                        ...
                        // toss obstacle away
                        this.app.fire(EventTypes.PLAY_HIT_EFFECT, this.entity.characterContainer.getPosition().clone().add(new pc.Vec3(0, 0, 1)));
                        this.app.fire(EventTypes.PLAY_SFX, Constants.AUDIO.INVINCIBLE_COLLISION);
    ...
    if(!Apicontroller['genuineCheck' + checkerID](0.004)) {
        console.error('other collision  ', contactResult.entity.path)
        this.pendingCollision = true;
    }
};
...
FreeRunLocation.prototype.updateFreeRunLeaderboard = function() {
    ...
    if(freeRunLeaderboard) {
        const currentPlayerPosition =  freeRunLeaderboard.findIndex(character => character.name === 'leaderboard_You'); 
        ...
        const nextPlayerPosition = freeRunLeaderboard.findIndex(character => character.name === 'leaderboard_You'); 
...
FreeRunLocation.prototype.createLeaderboard = function() {
    ...
    for (let i = 0; i < FreeRunLocation.freeRunLeaderboard.length; i++) {
        ...
        const padsList = nextLeaderboardSpot.children.map(childElement => childElement.findByName('Pad'));
        padsList.forEach(FreeRunLocation.freeRunLeaderboard[i].name === 'leaderboard_You' ? setPadColorPlayer : ((i % 2 === 0) ? setPadColorLight : setPadColorDark));
        
        const characterPlace = nextLeaderboardSpot.findByName('CharacterPlace');
        characterPlace.element.text = (i + 1);
        
        const characterName = nextLeaderboardSpot.findByName('CharacterName');
        characterName.element.text = LocalizationManager.getInstance().getLocalizedText(FreeRunLocation.freeRunLeaderboard[i].name); //FreeRunLocation.freeRunLeaderboard[i].name;
...
FreeRunLocation.prototype.closeClicked = function() {
     window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:FreeRunMode:Quit",
    });
    this.entity.hide();
};
...
FreeRunLocation.prototype.runClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:FreeRunMode:Run",
    });
...
FreeRunLocation.prototype.scrollToPlayerAvatar = function() {
    const playerIndex = FreeRunLocation.freeRunLeaderboard.findIndex(element => element.name === 'leaderboard_You');
...
LevelParser.prototype.postInitialize = function() {
    this.loadThemeJSON();
};



LevelParser.prototype.loadThemeJSON = function() {
    if(!this.levelSourceFile) return alert('Please drag theme JSON to LevelParser.levelSourceFile!');

    if(this.levelSourceFile.loaded)  {
        this.fire('parser:createGUI');
    } else {
        this.levelSourceFile.once('load', () =>  this.fire('parser:createGUI'));
        if(! this.levelSourceFile.loading) {
            this.app.assets.load( this.levelSourceFile);
        }
    }
};


LevelParser.prototype.buildCurrentBlock = function() {
    const themeBlockNames = Object.keys(this.levelSourceFile.resource);
    if(!this.currentBlockName || themeBlockNames.indexOf(this.currentBlockName) === -1) return console.error('Wrong or non-existent block name: ' + this.currentBlockName);
    const blockEntity = BlockBuilder.getInstance().buildBlockFromFileName(this.levelSourceFile.resource, this.currentBlockName);
    if(blockEntity) {
...
LevelParser.prototype.load = function(blockName) {
    if(!this.levelSourceFile) return console.error('Please drag theme JSON to LevelParser.levelSourceFile!');
    const worldJSON = this.levelSourceFile.resource;
    famobi.log('Loading block ' + blockName + '...');
    if(worldJSON[blockName]) {
        this.clear();
        this.currentBlockName = blockName;
        this.buildCurrentBlock();
    } else {
        console.error('Can not find block with name ' + blockName);
    }
...
LevelParser.prototype.rebuild = function() {
    famobi.log('Rebuilding block ' + this.currentBlockName + '...');
    this.clear();
    this.buildCurrentBlock();
};
...
LevelParserGui.prototype.initialize = function () {
    ...
    this.levelParserScript.once('parser:createGUI', () => {
...
LevelParserGui.prototype.createParserConfigObject = function () {
    ParserConfigObject = {
        ...
        testCheck: true,
        blockName: '-',
        load: () => this.levelParserScript.load(ParserConfigObject.blockName),
...
LevelParserGui.prototype.createDATGuiInstance = function () {
    ...
    this.addDropdownOption(datGuiInstance, ParserConfigObject, 'blockName', this._getBlockNames(), this.rebuildBlock, this).listen();
    ...
    // this.levelParserScript.on('attr:levelSourceFile', value => ParserConfigObject.blockName = value.name);
...
LevelParserGui.prototype._getBlockNames = function () {
    if (this.levelParserScript.levelSourceFile && this.levelParserScript.levelSourceFile.resource) {
        return Object.keys(this.levelParserScript.levelSourceFile.resource);
    }
    return ['-no theme JSON-'];
};


LevelParserGui.prototype.rebuildBlock = function (key, value) {
    ParserConfigObject.blockName = value;
    ParserConfigObject.load();
};
...
LevelParserGui.prototype.addDropdownOption = function (folder, optionHolder, optionKey, values, listener, listenerContext) {
    return folder.add(optionHolder, optionKey, values).onChange(value => listener.call(listenerContext, optionKey, value));
};
...
CoinsStorage.prototype.initialize = function() {
    ...
    /* initialize varibles */
    this._numCoins = LocalStorageController.getSavedValue(Constants.Storage.COINS) || 0;
...
MusicController.prototype.playMusic = function(key, stopOthers = true) {
    ...
    if(typeof this.musicPlayingInterval !== 'undefined') clearInterval(this.musicPlayingInterval);
    this.musicPlayingInterval = setInterval(this.resumeSuspendedMusic.bind(this), 1000);
};
...
MusicController.prototype.resumeSuspendedMusic = function() {
    if(!this.app.isHidden() && this.currentMusicKey) {
        const slot =  this.musicStorage.sound.slot(this.currentMusicKey);
        if(slot && !slot.isPlaying && slot.isStopped) {
            console.warn('[Music] Detected music suspended state, forcing playback...');
...
BlockBuilder.prototype.buildBlockFromFileName = function(themeJson, blockName) {
    const blockJson = themeJson[blockName];
    if(!blockJson) {
        console.error(`Block JSON '${blockName}' not found`);
        return null;
    }
    return this.buildBlock(blockName, blockJson);
};
...
WinLocation.prototype.buttonMultiplyRewardClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:Victory:MultiplyReward",
    });
...
WinLocation.prototype.buttonTapToProceedClicked = function () {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:Victory:Continue",
    });
...
MissionTarget.prototype.showMissionTargetData = function() {
    ...
    const missionTargets = (missionData.missionGoalTranslated && missionData.missionGoalTranslated[currentLocale]) ? missionData.missionGoalTranslated[currentLocale]: missionData.missionGoal;
...
Collectable.prototype.updateLetterMaterial = function(letterAsset) {
    if(this.level && letterAsset/* && letterAsset.loaded*/) { //check if not destroyed
        ...
        if(!letterAsset.loaded && !letterAsset.loading) {
            this.app.assets.load(letterAsset);
        } 
    }
};
...
LevelMissionController.prototype.prepareAvailableLetters = function() {
    const currentLocale = LocalizationManager.getInstance().getCurrentLocale();
    const currentMissionLevel = MissionsManager.getInstance().getCurrentMissionState().missionLevel;    
    let currentMissionWord = this.currentMissionData.missionGoal[pc.math.clamp(currentMissionLevel, 0, 2)];
    if(this.currentMissionData.missionGoalTranslated && this.currentMissionData.missionGoalTranslated[currentLocale]) {
        currentMissionWord = this.currentMissionData.missionGoalTranslated[currentLocale][pc.math.clamp(currentMissionLevel, 0, 2)]
    }
...
MissionsManager.prototype.initialize = function () {
    ...
    this.app.on(EventTypes.GAMEPLAY_LAUNCH, this.resetRevivesLimit, this);
};
...
MissionsManager.prototype.launchSelectedMode = function (freeRunMode = false, trackRestart = false, transitionDuration = 0.3) {
    ...
    /* tracking & analytics */
    if (window.famobi_analytics) {
        window.famobi_analytics.trackEvent(trackRestart ? 'EVENT_LEVELRESTART' : 'EVENT_LEVELSTART', { levelName: `${MissionsManager.getInstance().isFreeRunMode() ? "free-run" : MissionsManager.getInstance().getCurrentMissionIndex()}` });
    }
...
MissionsManager.prototype.resetRevivesLimit = function () {
    this.revivesLeft = window.MAX_REVIVES_PER_LEVEL !== undefined ? window.MAX_REVIVES_PER_LEVEL : -1;
}

MissionsManager.prototype.isReviveAvailable = function () {
    return this.revivesLeft > 0 || window.MAX_REVIVES_PER_LEVEL === undefined;
}

MissionsManager.prototype.useRevive = function () {
    if (window.MAX_REVIVES_PER_LEVEL !== undefined) {
        this.revivesLeft -= 1;
    }
}
...
CharactersManager.LEFT_CHARACTER_TRIED = false;
CharactersManager.RIGHT_CHARACTER_TRIED = false;
...
CharactersManager.prototype.isLeftCharacterTried = function() {
    return CharactersManager.LEFT_CHARACTER_TRIED;
};

CharactersManager.prototype.isRightCharacterTried = function() {
    return CharactersManager.RIGHT_CHARACTER_TRIED;
};


CharactersManager.prototype.setLeftCharacterTried = function(value) {
    CharactersManager.LEFT_CHARACTER_TRIED = value;
};

CharactersManager.prototype.setRightCharacterTried = function(value) {
    CharactersManager.RIGHT_CHARACTER_TRIED = value;
};
...
NotEnoughCoinsPopup.prototype.getFreeCoinsClicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:WatchVideoToEarnCoins:RewardedVideo",
    });
...
Apicontroller.isRewardedFeatureEnabled = function () {
    return window.famobi && window.famobi.hasFeature("rewarded");
};

Apicontroller.hideRewardedUIIfNoVideoAvailable = function () {
    return !Apicontroller.isRewardedFeatureEnabled();
};
...
Apicontroller.hasRewardedVideo = function () {
    ...
    if (window.famobi && window.famobi.hasFeature && window.famobi.hasFeature("rewarded") && window.famobi.hasRewardedAd)
        ...
        if (key === 'forced_mode') {
            return {
                "state": {
                    ...
                    "pass_distance": 100000
                },
                "override": {
                    // "hide_ui": ["coins", "mission_objective", "mission_progress"],
...
pc.AppBase.prototype.pauseGame = function (ignoreVisibilityAPI) {
    ...
    if (this.applicationPaused) return;
...
window.nextLevelEvent = () => {
    window.gotoLevel(MissionsManager.CURRENT_MISSION + 1);
};


window.gotoHomeEvent = () => {
    pc.AppBase.getApplication().fire(EventTypes.GAMEPLAY_EXIT);
    pc.AppBase.getApplication().fire(EventTypes.SHOW_UI_WINDOW, Constants.UI.MAIN_MENU);
};

window.gameEndEvent = () => {
    pc.AppBase.getApplication().fire(EventTypes.CharacterStatus.DEAD);
};


window.gotoLevel = (levelNumber) => {
    if (levelNumber > MissionsManager.getInstance().getTotalMissionsAmount()) {
        window.gotoHomeEvent();
        return;
    }
    ...
    if (levelNumber === -1) {
        ...
        const missionIndex = pc.math.clamp(levelNumber, 1, MissionsManager.getInstance().getTotalMissionsAmount());
        MissionsManager.getInstance().setMaxUnlockedMissionIndex(missionIndex);
        MissionsManager.getInstance().setCurrentMissionIndex(missionIndex);

        MissionsManager.getInstance().launchSelectedMode(false);
    }
};

window.enableSound = bool => {
    if (bool) {
        pc.AppBase.getApplication().fire("sfx:enable");
        pc.AppBase.getApplication().fire("music:enable");
    } else {
        pc.AppBase.getApplication().fire("sfx:disable");
        pc.AppBase.getApplication().fire("music:disable");
    }
};
...
LevelStatsManager.prototype.loadSavedValues = function() {
    const data = LocalStorageController.getSavedValue(Constants.Storage.LEVEL_DISTANCES);
    if(!data || data.length === 0) return;
    for(let i = 0; i < data.length; i++) {
        if(data[i]) {
            this.levelDistances[i] = data[i];
        } else {
            this.levelDistances[i] = 0;
        }
    }
}

LevelStatsManager.prototype.getLevelDistancesObject = function() {
    if(!this.levelDistances) return [];
    return this.levelDistances.slice();
};

LevelStatsManager.prototype.getTotalDistance = function() {
    return this.levelDistances.filter(value => !!value).reduce((curr, sum) => curr + sum, 0);
};

LevelStatsManager.prototype.saveLevelDistance = function() {
    const missionIndex = MissionsManager.getInstance().getCurrentMissionIndex();
    this.levelDistances[missionIndex] = Math.max(~~this.ranDistance || 0, this.levelDistances[missionIndex] || 0);
};
...
PowerupSpot.prototype.assignEntityMethods = function() {
    ...
    this.entity.pickup = function() {
        if(this.powerupEntity && !this.pickedUp) {
            ...
            window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
                eventName: "GA:Design",
                eventId: "PowerupCollected:" + this.powerupEntity.powerupType,
            });
...
ExclamationMarkIndidcator.prototype.initialize = function() {
    ...
    this.app.on(EventTypes.COINS_AMOUNT_CHANGED, this.updateVisibility, this);
    this.app.on(EventTypes.SELECT_ACTIVE_CHARACTER, this.updateVisibility, this);
};

ExclamationMarkIndidcator.prototype.postInitialize = function() {
...
ExclamationMarkIndidcator.prototype.updateVisibility = function(numCoins) {
    this.entity.element.opacity = this.isExclamationMarkVisible() ? 1 : 0.0;
};


ExclamationMarkIndidcator.prototype.isExclamationMarkVisible = function(dt) {
    let exclamationMarkVisible = false;

    const charactersConfig = CharactersManager.getInstance().getCharactersConfig();
    const charactersData = CharactersManager.getInstance().getCharactersData();

    Object.keys(charactersData).forEach(characterKey => {
        const characterData = charactersData[characterKey];
        const characterConfig = charactersConfig.characters.find(c => c.key === characterKey);
        const purchased = characterData.purchased;
        const unlockPrice = characterConfig.unlock;
        if(!purchased && CoinsStorage.getInstance().hasEnoughCoins(unlockPrice)) {
            exclamationMarkVisible = true;
        }
    });

    const currectCharacterKey = CharactersManager.prototype.getCurrentUnlockedCharacter();
    const currentCharacterConfig = charactersConfig.characters.find(c => c.key === currectCharacterKey); 
    const currentCharacterData = charactersData[currectCharacterKey]; 
    if(currentCharacterData.purchased) {
        const currentCharacterStuntLevels = currentCharacterData.levels;
        const currentCharacterUpgradePrices = currentCharacterConfig.stunts.map((stuntData, i) => stuntData.prices[currentCharacterStuntLevels[i]]);
        for(let upgradePrice of currentCharacterUpgradePrices) {
            if(upgradePrice > 0 && CoinsStorage.getInstance().hasEnoughCoins(upgradePrice)) {
                exclamationMarkVisible = true;
            }
        }
    }

    return exclamationMarkVisible;
};
...
ButtonLanguage.prototype.languageButtonCLicked = function() {
    window.famobi_analytics.trackEvent("EVENT_CUSTOM", {
        eventName: "GA:Design",
        eventId: "Button:Language:ChangeLanguageTo_" + this.languageCode
    });
...
LocalizationManager.prototype.loadSavedLocale = function() {
    setTimeout(() => {
        this._setCurrentLocale(this.app.i18n.findAvailableLocale(LocalStorageController.getSavedValue(Constants.Storage.LANGUAGE) || LocalizationManager.getDefaultLocale()));
    }, 0);
};
...
LocalizationManager.getDefaultLocale = function() {
    const famobiLang = window.famobi.getCurrentLanguage ? window.famobi.getCurrentLanguage() : null;
    return pc.AppBase.getApplication().i18n.findAvailableLocale(famobiLang || navigator.language || "en-US");
};
...
FamobiCopyright.prototype.initialize = function() {
    if(!famobi.hasFeature('copyright')) this.entity.enabled = false;
};
...
FamobiSafeArea.prototype.initialize = function () {
    ...
    setInterval(() => this._onCanvasResize(), 1000);
...
FamobiSafeArea.prototype._updateCanvasSizeAndPosition = function () {
    ...
    if(this.app.graphicsDevice.canvas.width == availableWidth * this.app.graphicsDevice.maxPixelRatio && this.app.graphicsDevice.canvas.height == availableHeight * this.app.graphicsDevice.maxPixelRatio) {
        // everything is fine, no need to update canvas size...
        return;
    }
...
screenFlashEffect.prototype.initialize = function() {
    ...
    this.app.on(EventTypes.LEVEL_EXIT, this.reset, this);
};
...
// gogglesPowerupStateManager.js
var GogglesPowerupStateManager = pc.createScript('gogglesPowerupStateManager');


GogglesPowerupStateManager.prototype.initialize = function() {
    this.app.on(EventTypes.POWERUP_ACTIVATED, this.handleActivated, this);
    this.app.on(EventTypes.POWERUP_DEACTIVATED, this.handleDeactivated, this);
};

GogglesPowerupStateManager.prototype.handleActivated = function(key) {
    if(key === Powerups.FIRST_PERSON_VIEW) {
        this.app.fire(EventTypes.FIRST_PERSON_GOGGLES_START_FLASHING);
    }
};


GogglesPowerupStateManager.prototype.handleDeactivated = function(key) {
    if(key === Powerups.FIRST_PERSON_VIEW) {
        this.app.fire(EventTypes.FIRST_PERSON_GOGGLES_STOP_FLASHING);
    }
};


GogglesPowerupStateManager.prototype.update = function(dt) {

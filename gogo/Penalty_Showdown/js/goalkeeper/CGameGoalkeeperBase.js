...
CGameGoalkeeperBase.prototype.clickStage = function(){
    s_oGameKeeper._oContainerGame.off("click",s_oGameKeeper._oListenerClickStage);
...
CGameGoalkeeperBase.prototype.unload = function () {

    s_oGameKeeper._iGameState = -1;
    s_oGameKeeper._oParentContainer.removeAllChildren();

    s_oGameKeeper._oScene.destroyWorld();
    // s_oGameKeeper._oScene = null;

    s_oGameKeeper.deactiveEventListeners();
...
CGameGoalkeeperBase.prototype.addImpulseToBall = function () {
...
CGameGoalkeeperBase.prototype.onHandKeeper = function (e) {
    var oHandKeeperBody = null;
if (s_oGameKeeper._oScene) {
    oHandKeeperBody = s_oGameKeeper._oScene.getHandKeeperBody();
}

    // var oHandKeeperBody = s_oGameKeeper._oScene.getHandKeeperBody();
    ...
    var oPos2D = s_oGameKeeper.convert3dPosTo2dScreen(oHandKeeperBody.position, s_oGameKeeper._oCamera);

    s_oGameKeeper._oGloves.setPosition(oPos2D.x, oPos2D.y);

    if (s_oGameKeeper._bPerfectSaved) {
...
CGameGoalkeeperBase.prototype.ballInHand = function (oPos) {
    ...
    //this._oBall.scale(this._oBall.getStartScale() - oPos2DBall.z);
    this._oBall.setPosition(oPos2DBall.x, oPos2DBall.y);
};
...
CGameGoalkeeperBase.prototype.timeReset = function () {
    if (this._fTimeReset > 0) {
        ...
        var iMaxLaunch = MATCH_LAUNCH[this._iLevel];
...
CGameGoalkeeperBase.prototype.displayShock = function (iTime, iXIntensity, iYIntensity) {
    ...
    createjs.Tween.get(s_oGameKeeper._oContainerGame).to({x: Math.round(Math.random() * xShifting), y: Math.round(Math.random() * yShifting)}, iTime).call(function () {
        createjs.Tween.get(s_oGameKeeper._oContainerGame).to({x: Math.round(Math.random() * xShifting * 0.8), y: -Math.round(Math.random() * yShifting * 0.8)}, iTime).call(function () {
            createjs.Tween.get(s_oGameKeeper._oContainerGame).to({x: Math.round(Math.random() * xShifting * 0.6), y: Math.round(Math.random() * yShifting * 0.6)}, iTime).call(function () {
                createjs.Tween.get(s_oGameKeeper._oContainerGame).to({x: Math.round(Math.random() * xShifting * 0.4), y: -Math.round(Math.random() * yShifting * 0.4)}, iTime).call(function () {
                    createjs.Tween.get(s_oGameKeeper._oContainerGame).to({x: Math.round(Math.random() * xShifting * 0.2), y: Math.round(Math.random() * yShifting * 0.2)}, iTime).call(function () {
                        createjs.Tween.get(s_oGameKeeper._oContainerGame).to({y: 0, x: 0}, iTime).call(function () {
                        });
                    });
                });
            });

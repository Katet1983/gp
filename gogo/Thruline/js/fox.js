...
fox = {
    ...
    // fit text to certain width (auto-shrink font size to make sure they fit into box)
    fittext2width: function(txt, wid = 200, maxfontsize = 40, minfontsize = 3) {
        txt.style.fontSize = maxfontsize;
        while (txt.width > wid) {
            txt.style.fontSize--;
        }
    },

    // fit text to box (will word wrap sentences and make auto-shrink font size to make sure they fit into box)
    fittext2box: function(txt, wid = 200, hei = 100, maxfontsize = 40, minfontsize = 3) {
        txt.style.wordWrap = true;
        txt.style.wordWrapWidth = wid;
        txt.style.fontSize = maxfontsize;
        txt.style.lineHeight = 1.2*txt.style.fontSize;
        while (txt.height > hei) {
            txt.style.fontSize--;
            txt.style.lineHeight = 1.2*txt.style.fontSize;
        }
    },
    ...
    // USAGE : let c = fox.hex2color("#000000");
    hex2color: function(hexstring) {
    ...
    copy2clipboard: function(text) {
        if (navigator.clipboard) {
            try {
                ...
                console.warn("clipboard.writeText failed!", ex);
                return false;
            }
        } else {
            if (window.clipboardData && window.clipboardData.setData) {
                ...
                try {
                    ...
                    console.warn("execCommand copy to clipboard failed!", ex);
                    return false;
                } finally {
    ...
    // input JSON or txt file
    // Usage: fox.inputfile((res)=> { g.playerstats = res })
    inputfile: function(callback) {
        if (!g.foxfileInput) {
            // create HTML file input element (if not exists)
            let element = document.createElement('div');
            element.innerHTML = '<input type="file">';
            g.foxfileInput = element.firstChild;
            g.foxfileInput.addEventListener('change', function () {
                let file = g.foxfileInput.files[0];
                // make sure it's txt/json file
        ...
        g.foxfileInput.click();
    },

    // add hitzone
    // usage: a.hitzone = fox.addhitzone(a,g.hitzone[it.n]);
    addhitzone: function(it, hitzonearray) {
        it.hitzone = fox.clone(hitzonearray);
        // flip x values?
        if (it.flipX < 0) {
            if (it.hasOwnProperty('x2')) {
                let foo = it.x1;
                it.x1 *= flipX;

            }
            if (it.hasOwnProperty('x2')) it.x2 *= flipX;
            if (it.hasOwnProperty('x')) it.x *= flipX;
            if (it.hasOwnProperty('y1')) it.y1 *= flipY;
            if (it.hasOwnProperty('y2')) it.y2 *= flipY;
            if (it.hasOwnProperty('y')) it.y *= flipY;
        }
        return it.hitzone;
    },

    // glow display object
    addglow: function(it, amount = 5, color = 0xffffff) {
        it.glowfilter = new PIXI.filters.GlowFilter({distance: amount*2, color, outerStrength: amount});
        if (!it.filters) {
            it.filters = [it.glowfilter];
        } else {
            it.filters.push(it.glowfilter);
        }
        return it.glowfilter;
    },
    ...
    blendmode: function(it,blend = PIXI.BLEND_MODES.ADD) {
        if (it.isfoxpic) {
            ...
            fox.trace('fox.blendmode failed');
        }
    },
    ...
    */
    ...
    // tell a foxinstance to wander around
    ...
    wander: function(it,params) {
        ...
        // add wander function into this foxinstance
    ...
    brightfade: function(mc, duration = 400, delay = 0, glowdistance = 80) {
        ...
        if (glowdistance > 0) {
            ...
            mc.brightfade_tweenglowfilter.once('start',()=> {
                mc.visible = true;
                mc.filters = arr;
            })
        }
    ...
    hijackconsolelog: function() {
        ...
        console.log = function(message) {
            if (typeof message === 'string') {
                ...
                msg = msg.replace(/['"]+/g, '')
                g.log += '\n' + msg;
                if (g.showinglog) fox.updateshowlog();
            }
    ...
    // you can activate by tapping top-right corner of the game screen at loading screen
    ...
    updateshowlog: function() {
        g.logtext.text = g.log;
    ...
    // FOR LOCAL TESTING make all children of a container draggable with mouse/keyboard
    ...
    draggablechildren: function(container) {
        ...
        for (let i = 0; i < container.children.length; i++) {
            fox.draggable(container.children[i],true,false,false,true);
        }
    },
    ...
    //        you can also add movement limits
    //        - dragging_xmin
    //        - dragging_xmax
    //        - dragging_ymin
    //        - dragging_ymax
    // Set 'reportposition' to TRUE to get object position (x,y) report
    // Example:     fox.draggable(t.scrollbarbutton,false,true,false,true);
    //              t.scrollbarbutton.dragging_ymin = 65;
    //              t.scrollbarbutton.dragging_ymax = 207;
    draggable: function(it, usekeyboard = false, lockhorizontal = false, lockvertical = false, reportposition = false) {
        it.interactive = true;
        it.presstime = 0;
        it.lockhorizontal = lockhorizontal;
        it.lockvertical = lockvertical;
        it.reportposition = reportposition;
        it
    ...
    draggingend: function(it,e) {
        ...
        // report position
        if (it.reportposition) fox.trace('x:'+it.x+',y:'+it.y);
    },
    ...
    draggingmove: function(it,e) {
        if (it.event_dragging) {
            ...
            if (it.dragging_xmin !== undefined) it.x = Math.max(it.dragging_xmin,it.x);
            if (it.dragging_xmax !== undefined) it.x = Math.min(it.dragging_xmax,it.x);
            if (it.dragging_ymin !== undefined) it.y = Math.max(it.dragging_ymin,it.y);
            if (it.dragging_ymax !== undefined) it.y = Math.min(it.dragging_ymax,it.y);
    ...
    updateignorepauseitems: function() {
        // update foxinstance loops that 'ignore-pause'
        for (let i = 0; i < g.activeitems.length; i++) {
    ...
    // check for local storage accessibility
    ...
    spawn: function (name, x, y, parent, params) {
        ...
        if (it == null) {
            ...
            if (name in g.foxclip) {
                it = fox.attachclip(name, x, y, parent, params);
    ...
    // fadescreen
    fadescreen: function(fadeout = true, color = 0xFFFFFF, duration = 1000, delay = 0) {
    ...
    // Known issue: calling delayaction during pause may result in wrong delay execution. Avoid calling fox.delayaction
    // during pause!! Call it BEFORE fox.pausegame() of AFTER fox.resumegame()
    delayaction: function(delay, callback, ignorepause = false) {
        let ID = g.delayactionID;
        let timenow = delay > 0 ? Date.now() : g.gameframenow;
        // calling delayaction during paused, set timenow back to the time when fox.pausegame is called
        if (g.paused && !ignorepause) timenow = delay > 0 ? g.pausetime : g.pauseframetime;
        g.delayactions[ID] = { delay, callback, ignorepause, starttime:timenow};
        g.delayactionID++;
        return ID;
    },
    ...
    repeataction: function(delay, callback, count = -1, ignorepause = false) {
        ...
        let timenow = delay > 0 ? Date.now() : g.gameframenow;
        // calling repeataction during paused, set timenow back to the time when fox.pausegame is called
        if (g.paused && !ignorepause) timenow = delay > 0 ? g.pausetime : g.pauseframetime;
        g.repeatactions[ID] = { delay, callback, count, ignorepause, starttime:timenow };
        g.repeatactionID++;
        return ID;
    },
    ...
    // run scene
    runscene: function(scene, fade = false, duration = 500, color = 0xFFFFFF) {
    ...
    playsound: function(name, volume) {
        if (!GameSnacks.audio.isEnabled()) return;
        if (g.mutesfx) return;
        if (volume === undefined) volume = g.sfxvolume;
        if (name in g.sfx) {
    ...
    playsting: function(name, delayoffset = 500, bgvolume = 0) {
        ...
        if (g.bgmusicname != '') {
            ...
            fox.delayaction(g.sfx[name].duration*1000-delayoffset,()=> { fox.fadesound(g.bgmusicname,1,500,0) })
        }
    },
    ...
    // checkcoll between 2 mc that have hitzone array containing hitboxes/hitcircles.
    // Rules: - both mc must be in the same coordinate system
    // Note: use Flash 'hitzone editor' to easily set up hitzone arrays.
    cekhitzonecollision: function(A,B) {
        if (!A.hasOwnProperty('hitzone') || !B.hasOwnProperty('hitzone') || A.hitzone === null || B.hitzone == null) return false;
        let Ax1,Ax2,Ay1,Ay2,Ax,Ay,Bx1,By1,Bx2,By2,Bx,By;
        // draw hitzone?
        if (g.drawhitzone) { fox.drawhitzone(A); fox.drawhitzone(B); }
        // First, we check for bounding box collision
        let AA = A.hitzone[A.hitzone.length-1];
        let BB = B.hitzone[B.hitzone.length-1];
        Ax1 = A.flipX > 0 ? A.x + AA.x1 : A.x - AA.x2;
        Ax2 = A.flipX > 0 ? A.x + AA.x2 : A.x - AA.x1;
        Ay1 = A.flipY > 0 ? A.y + AA.y1 : A.y - AA.y2;
        Ay2 = A.flipY > 0 ? A.y + AA.y2 : A.y - AA.y1;
        Bx1 = B.flipX > 0 ? B.x + BB.x1 : B.x - BB.x2;
        Bx2 = B.flipX > 0 ? B.x + BB.x2 : B.x - BB.x1;
        By1 = B.flipY > 0 ? B.y + BB.y1 : B.y - BB.y2;
        By2 = B.flipY > 0 ? B.y + BB.y2 : B.y - BB.y1;
        if (!fox.rectangleintersection(Ax1,Ay1,Ax2,Ay2,Bx1,By1,Bx2,By2)) return false;
        // if both a & b only have 1 box in them
        if (A.hitzone.length === 1 && B.hitzone.length === 1) return true;
        // Then we check hitzone in array one by one
        for (let i = 0; i < A.hitzone.length; i++) {
            // skip A's bounding box
            if (A.hitzone.length > 1 && i === A.hitzone.length-1) continue;
            let C = A.hitzone[i];
            for (let f = 0; f < B.hitzone.length; f++) {
                // skip B's bounding box
                if (B.hitzone.length > 1 && f === B.hitzone.length-1) continue;
                let D = B.hitzone[f];
                if (C.hasOwnProperty('x2')) {
                    // C is a hitbox
                    Ax1 = A.flipX > 0 ? A.x + C.x1 : A.x - C.x2;
                    Ax2 = A.flipX > 0 ? A.x + C.x2 : A.x - C.x1;
                    Ay1 = A.flipY > 0 ? A.y + C.y1 : A.y - C.y2;
                    Ay2 = A.flipY > 0 ? A.y + C.y2 : A.y - C.y1;
                } else {
                    // C is a hitcircle
                    Ax = A.x + A.flipX * C.x;
                    Ay = A.y + A.flipY * C.y;
                ...
                if (D.hasOwnProperty('x2')) {
                    // D is a hitbox
                    Bx1 = B.flipX > 0 ? B.x + D.x1 : B.x - D.x2;
                    Bx2 = B.flipX > 0 ? B.x + D.x2 : B.x - D.x1;
                    By1 = B.flipY > 0 ? B.y + D.y1 : B.y - D.y2;
                    By2 = B.flipY > 0 ? B.y + D.y2 : B.y - D.y1;
                } else {
                    // D is a hitcircle
                    Bx = B.x + B.flipX * D.x;
                    By = B.y + B.flipY * D.y;
                ...
                if (C.hasOwnProperty('x2')) {
                    if (D.hasOwnProperty('x2')) {
                        if (fox.rectangleintersection(Ax1,Ay1,Ax2,Ay2,Bx1,By1,Bx2,By2)) return true;
                    } else {
                        if (fox.rectanglecircleintersection(Ax1,Ay1,Ax2,Ay2,Bx,By,D.r)) return true;
                    }
                } else {
                    if (D.hasOwnProperty('x2')) {
                        if (fox.rectanglecircleintersection(Bx1,By1,Bx2,By2,Ax,Ay,C.r)) return true;
                    } else {
                        if (fox.circlecircleintersection(Ax,Ay,C.r,Bx,By,D.r)) return true;
                    }
                }
            }
        }
    ...
    // draw hitzone
    drawhitzone: function(mc) {
        if (!mc.hasOwnProperty("hitzone") || mc.hitzone == null) return;
        if (!mc.hitzonedrawingcontainer) mc.hitzonedrawingcontainer = fox.makecontainer(0,0,mc);
        mc.hitzonedrawingcontainer.removeChildren();
        let parent = mc.hitzonedrawingcontainer;
        let hitzone = mc.hitzone;
        for (let i = 0; i < hitzone.length; i++) {
            let a = hitzone[i];
            if (hitzone.length === 1 || i < hitzone.length-1) {
                if (a.hasOwnProperty('x2')) {
                    // draw box
                    fox.makebox(a.x1, a.y1, a.x2 - a.x1, a.y2 - a.y1, parent, 0xff0000, 1,true, 1);
                } else {
                    // draw circle
                    fox.makecircle(a.x,a.y,a.r, parent, 0xff0000, 1, true, 1);
                }
            }
            // if there's only 1 box, no need to continue making bounding box because there's no use
            if (hitzone.length === 1) break;
            // draw bounding box (last box in the hitzone array is the hitzone's bounding box)
            if (i === hitzone.length-1) {
                fox.makebox(a.x1, a.y1, a.x2 - a.x1, a.y2 - a.y1, parent, 0xffcc00, 1,true,1);
            }
        }
    },
    ...
    // make box (using white pixel)
    // usage: fox.makeboxfrompixel(0,0,g.screenwid,g.screenhei,null,0xff0000,0.5);
    makeboxfrompixel: function(x, y, wid, hei, parent = g.scene, color = 0xff0000, alpha = 1) {
        let it = fox.attachpic('whitepixel',x,y,parent);
        it.width = wid;
        it.height = hei;
        it.alpha = alpha;
        fox.tint(it,color);
    ...
    // make circle
    // usage: fox.makecircle(0,0,50,null,0xff0000,0.5,true);
    makecircle: function(x, y, radius, parent = g.scene, color = 0xff0000, alpha = 1, outline = false, outlinesize = 2) {
        let it = new PIXI.Graphics();
        if (outline) {
            // draw just the outline of the circle
            it.lineStyle({width: outlinesize, color, alpha, alignment: 0.5});
        } else {
            // draw solid box
            it.beginFill(color);
        }
        it.drawCircle(x,y,radius);
        it.alpha = alpha;
        parent.addChild(it);
        return it;
    },
    ...
    drawcrosshair: function (parent,x,y,size) {
        ...
        ch.lineStyle(20, 0xFFFFFF, 1).moveTo(x-hsize,y).lineTo(x+hsize,y).moveTo(x,y-hsize).lineTo(x,y+hsize);
        hsize = size;
        ch.lineStyle(10, 0xCC0000, 1).moveTo(x-hsize,y).lineTo(x+hsize,y).moveTo(x,y-hsize).lineTo(x,y+hsize);
        parent.addChild(ch);
    },
    ...
    attachani: function (name, x, y, parent, params) {
        ...
        let it = null;
        if (g.foxani.hasOwnProperty(name) && g.foxani[name]) {
    ...
    // usage: looping > 0=none, 1=loop_normal, 2=loop_yoyo
    //        oncompletekill > true/false
    attachclip: function (name, x, y, parent, params) {
        x = typeof x !== 'undefined' ? x : g.hscreenwid;
        y = typeof y !== 'undefined' ? y : g.hscreenhei;
        parent = parent || g.scene;
        let looping = 1;
        ...
        if (name in g.foxclip) {
            ...
            fox.alert("fox.attachclip ERROR! "+name+" not found!");
        }
        return it;
    },
    ...
    // attach pic (will create sprite inside a foxinstance container)
    ...
    // attach sprite (will create sprite without foxinstance container
    ...
    attachbutton: function (name, x, y, parent, callback) {
        ...
        if (name in g.foxpic) {
            button = fox.makecontainer(x,y,parent);
            button.pic = fox.attachpic(name,0,0,button, {heaven:true}); // make button pic pixi-heaven
            button.name = name;
            button.interactive = button.enabled = true;
            button.animating = false;
            button.callback = callback;
            button.on('pointertap',()=> {
                if (button.enabled && !button.animating) {
                    button.animating = true;
                    fox.tweenremoveallfrom(button.pic);
    ...
    // attach button containing text
    // usage: fox.attachbutton('buttonplay',x,y,null,()=> t.buttonclicked());
    // NOTE: to add text to button, you can do fox.attachtext('SKIP',0,0,t.button.pic,textstyle,true);
    attachbuttontext: function (text, style, pic, x, y, parent, callback, textoffsetx, textoffsety, width) {
        x = typeof x !== 'undefined' ? x : g.hscreenwid;
        y = typeof y !== 'undefined' ? y : g.hscreenhei;
        parent = parent || g.scene;
        let button = null;
        if (pic in g.foxpic) {
            button = fox.makecontainer(x,y,parent);
            button.pic = fox.attachpic(pic,0,0,button, {heaven:true}); // make button pic pixi-heaven
            // add text
            button.text = fox.attachtext(text,textoffsetx,textoffsety,button.pic,style);
            button.name = pic;
            button.interactive = button.enabled = true;
            button.animating = false;
            fox.bringtotop(button.text);
            ...
            fox.alert("fox.attachbuttontext ERROR! "+name+" not found!");
        }
        return button;
    },
    ...
    attachtext: function (text, x, y, parent, textstyle, center) {
        ...
        it.x = x;
        it.y = y;
        it.resolution = g.textresolution;
    ...
    // slide fade object (time in miliseconds)
    // usage: fox.slidefade(button,700,700,true,0,-50);
    // NOTE: give yoffset/xoffset negative value to change slide direction
    slidefade: function(it,duration = 700, delay = 0,fadein = true, yoffset = 50, xoffset = 0) {
        fox.tweenremoveallfrom(it);
        let targetalpha = fadein ? 1 : 0;
        let targetY = fadein ? it.y : it.y-yoffset;
        let targetX = fadein ? it.x : it.x-xoffset;
        let easing = fadein ? g.easing.outExpo() : g.easing.inCirc();
        if (fadein) {
            it.alpha = 0;
            if (yoffset !== 0) it.y += yoffset;
            if (xoffset !== 0) it.x += xoffset;
        }
        it.visible = true;
        if (yoffset !== 0) fox.tweenY(it,it.y,targetY,duration,delay,easing);
        if (xoffset !== 0) fox.tweenX(it,it.x,targetX,duration,delay,easing);
        let tw = fox.tweenalpha(it,it.alpha,targetalpha,duration,delay,g.easing.linear());
        return tw;
    },
    ...
    // kill all children in a container/foxinstance
    ...
    trace: function (isi, onlylocal = true, textcolor = '#C10202', backgroundcolor = '#66FF00') {
        ...
        try {
            ...
            if (Object.prototype.toString.call(isi).slice(0, 7) === '[object') {
                ...
                // send log to Unity
                if (!g.localtestingChromeEmulation && !g.isdesktop) window.location.href = "uniwebview://debuglog?msg="+res;
            } else {
                // normal
                console.log(prefix + isi + ' ', 'background:'+backgroundcolor+';color:'+textcolor);
                // send log to Unity
                if (!g.localtestingChromeEmulation && !g.isdesktop) window.location.href = "uniwebview://debuglog?msg="+isi;
            }
        } catch(e) {
            console.log(isi);
        }
    ...
    cekcollrectanglecircle: function (rect,circle) {
    ...
    rectanglecircleintersection: function(ax1, ay1, ax2, ay2, cx, cy, radius) {
        // Find the closest point to the circle within the rectangle
        let closestX = Math.clamp(cx, ax1, ax2);
        let closestY = Math.clamp(cy, ay1, ay2);
    ...
    circlecircleintersection: function(c1x, c1y, c1r, c2x, c2y, c2r) {

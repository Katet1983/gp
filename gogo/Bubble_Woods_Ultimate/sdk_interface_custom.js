...
SDK_INTERFACE_SETTINGS.version = "1.0.8";
	...
	return new Promise(function(resolve, reject) {
		...
		switch(event) {
			...
			case "EVENT_TOTALSCORE":
				SDK_INTERFACE_HELPERS.GameSnacks.score.update(params.totalScore);
				break;
			...
			case "EVENT_LEVELRESTART":
				SDK_INTERFACE_HELPERS.GameSnacks.score.reset();
				SDK_INTERFACE_HELPERS.GameSnacks.score.update(0);
				return window.famobi.showInterstitialAd("button:main:start").then(() => {
					resolve(event, params);
				});
			
			case "EVENT_PAUSE":
				return window.famobi.showInterstitialAd("button:game:pause").then(() => {
					resolve(event, params);
				});
				break;

			case "EVENT_CUSTOM":
				if(params.eventName === "EVENT_LEVELSUCCESS") {
					SDK_INTERFACE_HELPERS.GameSnacks.game.levelComplete(params.levelName);
				} else if(params.eventName === "EVENT_LEVELFAIL") {
					SDK_INTERFACE_HELPERS.GameSnacks.game.gameOver();
				}
				break;

...
'use strict'; {
    ...
    window.DOMElementHandler = class DOMElementHandler extends self.DOMHandler {
        ...
        _OnCreate(e) {
            const elementId = e["elementId"];
            const elem =
                this.CreateElement(elementId, e);
            ...
            if (this._autoAttach) {
                const actualHtmlIndex = this.GetRuntimeInterface().GetAvailableHTMLIndex(wantHtmlIndex);
                elementState.SetActualHTMLIndex(actualHtmlIndex);
                const parent = this.GetRuntimeInterface().GetHTMLWrapElement(actualHtmlIndex);
        ...
        _OnHTMLLayersChanged() {
            ...
            for (const elemState of this._elementMap.values()) {
                const wantHtmlIndex = this.GetRuntimeInterface().GetAvailableHTMLIndex(elemState.GetWantHTMLIndex());
                const actualHtmlIndex = elemState.GetActualHTMLIndex();
                if (wantHtmlIndex !== -1 && actualHtmlIndex !== -1 && wantHtmlIndex !== actualHtmlIndex) {
                    const elem = elemState.GetElement();
                    elem.remove();
                    const parent = this.GetRuntimeInterface().GetHTMLWrapElement(wantHtmlIndex);
                    parent.appendChild(elem);
        ...
        _OnSetFocus(e) {
            const elem =
                this._GetFocusElement(this.GetElementById(e["elementId"]));
        ...
        GetElementById(elementId) {
            const elementState =
                this._elementMap.get(elementId);
...
'use strict'; {
	...
	self.addEventListener("message", () =>
	{
		...
		catch (err)
		{
            ...
            const w = new Worker(URL.createObjectURL(workerScriptBlob), {get type() {
    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this._exportType === "cordova" && this._useWorker)
                if (isAndroid) {
                    ...
                    if (!chromeVer || !(parseInt(chromeVer[1], 10) >= 90)) this._useWorker =
                        false
                }
            if (this.IsAnyWebView2Wrapper()) self["chrome"]["webview"].addEventListener("message", e => this._OnWrapperMessage(e.data));
            else if (this._exportType === "macos-wkwebview") self["C3WrapperOnMessage"] = msg => this._OnWrapperMessage(msg);
        ...
        IsAnyWebView2Wrapper() {
            return this.IsWindowsWebView2() ||
                this._exportType === "xbox-uwp-webview2"
        }
        async _Init(opts) {
            if (this._useWorker) {
        ...
        _GetWorkerURL(url) {
            ...
            else if (this._exportType === "playable-ad" && this._localFileBlobs.hasOwnProperty(url)) ret =
        ...
        GetCssDisplayMode() {
            ...
            const standaloneExportTypes = new Set(["cordova", "nwjs", "macos-wkwebview"]);
            if (standaloneExportTypes.has(exportType)) return "standalone";
            if (window.matchMedia("(display-mode: fullscreen)").matches) return "fullscreen";
            else if (window.matchMedia("(display-mode: standalone)").matches) return "standalone";
            else if (window.matchMedia("(display-mode: minimal-ui)").matches) return "minimal-ui";
        ...
        async _InitDOM(opts, port2) {
            ...
            if (Array.isArray(opts.workerDependencyScripts)) {
                const workerDependencyScripts =
                    [...opts.workerDependencyScripts].map(s => s instanceof Blob ? URL.createObjectURL(s) : s);
            ...
            this._localRuntime =
                self["C3_CreateRuntime"](runtimeOpts);
        ...
        _OnUpdateCanvasSize(e) {
            ...
            if (count < this._canvasLayers.length)
                while (this._canvasLayers.length > count) {
                    ...
                    htmlWrap.remove();
                    if (this._useWorker && !immediate) this._pendingRemoveElements.push(canvas);
                    else canvas.remove()
        ...
        _DoUpdateHTMLElementsZOrder() {
            ...
            for (; i < len; ++i) {
                ...
                if (es.GetActualHTMLIndex() !== curHtmlIndex) {
                    this._DoUpdateHTMLElementsZOrderOnHTMLLayer(curHtmlIndex,
        ...
        _DoUpdateHTMLElementsZOrderOnHTMLLayer(htmlIndex, arr) {
            ...
            const existingChildren = Array.from(htmlWrap.children).filter(elem => newChildrenSet.has(elem));
            let i = 0,
        ...
        _OnEventFromRuntime(e) {
            ...
            if (responseId === null) return ret;
                ...
                ret.then) ret.then(result => this._PostResultToRuntime(responseId, true, result)).catch(err => {
        ...
        _RequestAnimationFrame() {
            if (this._rafId ===
                -1 && this._rafCallbacks.size > 0) this._rafId = requestAnimationFrame(this._rafFunc)
        }
        ...
        _OnWrapperMessage(msg) {
            if (msg === "entered-fullscreen") {
                ...
                if (type === "wrapper-init-response") {
                    ...
                    "extension-message") this.PostToRuntimeComponent("runtime", "wrapper-extension-message", msg);
        ...
        _SendWrapperMessage(o) {
            if (this.IsAnyWebView2Wrapper()) window["chrome"]["webview"]["postMessage"](JSON.stringify(o));
        ...
        _DataURIToBinaryBlobSync(datauri) {
            ...
            return this._BinaryStringToBlob(o.data,
                o.mime_type)
        }
...
'use strict'; {
    ...
    const keyboardInputElementTagNames = new Set(["input",
    ...
    self["C3_RasterSvgImageBlob"] = async function(blob, imageWidth,
        imageHeight, surfaceWidth, surfaceHeight) {
        const img = await BlobToImage(blob);
        const canvas = document.createElement("canvas");
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            iRuntime.AddRuntimeComponentMessageHandler("runtime",
                "load-webfonts", e => this._OnLoadWebFonts(e));
            ...
            iRuntime.AddRuntimeComponentMessageHandler("runtime",
                "post-to-debugger", e => this._OnPostToDebugger(e));
            ...
            iRuntime.AddRuntimeComponentMessageHandler("runtime",
                "enable-device-motion", () => this._AttachDeviceMotionEvent());
        ...
        _OnBeforeStartTicking() {
            ...
            } else document.addEventListener("visibilitychange", () => this._OnVisibilityChange(document.visibilityState === "hidden"));
        ...
        Attach() {
            ...
            window.addEventListener("mousedown", e => this._OnMouseEvent("mousedown", e, DISPATCH_SCRIPT_ONLY));
            window.addEventListener("mousemove", e => this._OnMouseEvent("mousemove", e, DISPATCH_SCRIPT_ONLY));
            window.addEventListener("mouseup", e => this._OnMouseEvent("mouseup",
                e, DISPATCH_SCRIPT_ONLY));
            window.addEventListener("dblclick", e => this._OnMouseEvent("dblclick", e, DISPATCH_RUNTIME_AND_SCRIPT));
            window.addEventListener("wheel", e => this._OnMouseWheelEvent("wheel", e, DISPATCH_RUNTIME_AND_SCRIPT));
            window.addEventListener("pointerdown", e => {
                this._HandlePointerDownFocus(e);
                this._OnPointerEvent("pointerdown", e)
            });
            if (this._iRuntime.UsesWorker() && typeof window["onpointerrawupdate"] !== "undefined" && self === self.top) window.addEventListener("pointerrawupdate", e => this._OnPointerRawUpdate(e));
                ...
                if (this._enableAndroidVKDetection)
                    if (this._lastWindowWidth === width && height < this._lastWindowHeight) {
                        ...
                        if (this._virtualKeyboardHeight > 0) {
                            this._virtualKeyboardHeight = 0;
                            this._OnAndroidVirtualKeyboardChange(height,
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen["orientation"] && screen["orientation"]["lock"]) screen["orientation"]["lock"](orientation).catch(err => console.warn("[Construct] Failed to lock orientation: ",
                err));
            else try {
                let result = false;
        ...
        _OnKeyEvent(name, e) {
            ...
            if (this._iRuntime.GetExportType() === "nwjs" && e.key === "u" && (e.ctrlKey || e.metaKey)) e.preventDefault();
            if (this._isExportToVideo) return;
        ...
        _IsElementCanvasOrDocument(elem) {
            return !elem ||
                elem === document || elem === window || elem === document.body || elem.tagName.toLowerCase() === "canvas"
        }
        ...
        async _OnRasterSvgImage(e) {
            ...
            const canvas = await self["C3_RasterSvgImageBlob"](blob,
                imageWidth, imageHeight, surfaceWidth, surfaceHeight);
            let ret;
        ...
        _OnScreenReaderTextEvent(e) {
            ...
            if (type === "create") {
                ...
                const p = document.getElementById("c3-sr-" +
                    e["id"]);
        ...
        _DoUpdateExportVideoProgressMessage() {
            this._exportVideoUpdateTimerId = -1;
            const headerElem =
                document.getElementById("exportToVideoMessage");
...
'use strict'; {
    ...
    const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
        ...
        _OnReadyForSWMessages() {
            ...
            window["OfflineClientInfo"]["SetMessageCallback"](e => this.PostToRuntime("sw-message",
                e["data"]))
        }
        ...
        _OnNavigate(e) {
            ...
            if (type === "back")
                ...
                else if (exportType === "preview" || this._iRuntime.IsAnyWebView2Wrapper()) window.open(url, "_blank");
                ...
                const tag = e["tag"];
                if (self["cordova"] &&
                    self["cordova"]["InAppBrowser"]) self["cordova"]["InAppBrowser"]["open"](url, "_system");
                else window.open(url, tag)
            }
        }
        _OnRequestFullscreen(e) {
            if (this._iRuntime.IsAnyWebView2Wrapper() || this._exportType === "macos-wkwebview") {
    ...
    self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
        ...
        async _AddEffect(e) {
            ...
            for (const tag of tags) {
                ...
                else if (type === "analyser") effect = new self.C3AudioAnalyserFX(this,
                    ...params);
...
'use strict'; {
    ...
    const HANDLER_CLASS = class PlatformInfoDOMHandler extends self.DOMHandler {
        ...
        async _GetSafeAreaInset() {
            ...
            if (AndroidNotch) try {
                return await Promise.all([GetAndroidInsetTop(AndroidNotch), GetAndroidInsetRight(AndroidNotch), GetAndroidInsetBottom(AndroidNotch), GetAndroidInsetLeft(AndroidNotch)])
            } catch (err) {
                ...
                const ret = [computedStyle.getPropertyValue("--temp-sai-top"), computedStyle.getPropertyValue("--temp-sai-right"),
                    computedStyle.getPropertyValue("--temp-sai-bottom"), computedStyle.getPropertyValue("--temp-sai-left")
                ].map(str => {

...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _GetSafeAreaInset() {
        ...
        if (a) try {
            return await Promise.all([GetAndroidInsetTop(a), GetAndroidInsetRight(a), GetAndroidInsetBottom(a), GetAndroidInsetLeft(a)])
        } catch (c) {
            return console.error("[Construct] Failed to get Android safe area inset: ", c), [0, 0, 0, 0]
        } else {
            var b = document.body;
            a = b.style;
            a.setProperty("--temp-sai-top",

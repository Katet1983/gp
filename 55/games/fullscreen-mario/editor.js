...
function setEditorLibrary() {
    ...
    window.editor = {
        ...
        offset: {
            x: unitsizet2
        }, // max is set upon running, if actually needed
        settings: {
            night: false,
            setting: "Overworld",
            alt: false
...
function setEditorControls(names) {
    ...
    for (i in names) {
        ...
        div = createElement("div", {
            ...
            className: "control",
            style: {
                backgroundImage: "url(Theme/" + name + ".gif)"
            },
...
function editorAddBottomPreview(bottombar, name, ref) {
    ...
    var width = ref.width,
        ...
        canvas = proliferate(getCanvas(width * unitsizet2, height * unitsizet2), {
            ...
            reference: ref,
            style: {
                marginLeft: -roundDigit(width / 2, scale) + "px"
            },
            onclick: editorSetCurrentThing
        }),
...
function editorSetCurrentThingFromName(name, sameargs) {
    editorSetCurrentThing({
        target: {
            name: name || editor.current_thing_name
        }
    }, sameargs);
}
...
function editorClose(inmap) {
    ...
    var ids = ["maplines", "sidebar", "bottombar", "scrollers"],
        i;
